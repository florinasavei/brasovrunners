...
9. The run's page offers the declaration under the route at `#declaratie` only when the organizer offered it on a GROUP_RUN of surface ASPHALT or TRAIL, an approved text of that surface and an approved privacy notice are in force, the event is published and not cancelled, and its start is at most 60 minutes past. The section is named by an h2 heading in both languages, and its button is at least 44 px tall (2026-09-25, `DECISIONS.md` §393).
10. In the event editor, the offer checkbox shows for a group run only. It is disabled without an approved text of the chosen surface, ticks itself when the surface becomes Trail, and keeps a stored answer until the surface changes. Its help line says the button needs an approved privacy notice (2026-09-25, `DECISIONS.md` §393).
...
61. The difficulty pill draws its level as a scale of Material's FitnessCenter dumbbell, one per level of the closed set in a single aria-hidden `<svg>`. EASY lights one, MODERATE two, HARD three; the rest are drawn at the theme's disabled opacity, on the event page's route row, the listing card's compact pills and the backoffice's event cards (2026-09-25, `DECISIONS.md` §399).
62. The difficulty pill's visible word is the level alone («Mediu» / «Moderate»). A screen reader then hears the field's catalogue name as visually hidden text through `GlyphChip`'s `srSuffix` («— Dificultate» / «— Difficulty»), never through an `aria-label` on the chip's roleless `<div>`, in both languages and for every level (2026-09-25, `DECISIONS.md` §399).
63. The difficulty levels are declared once (`difficulty-levels.ts`) and checked against the database enum at typecheck. The icon's width, its accepted levels and the glyph registry's `difficulty:*` entries all derive from that list, and the registry stays readable from a Server Component (2026-09-25, `DECISIONS.md` §399).
64. At 320 px and 360 px the listing card's compact difficulty pill shows its word and the scale with no sideways scroll; its measured width, 103.9 px at both, is recorded (2026-09-25, `DECISIONS.md` §399).
65. The listing's filter row carries a `DENSITY.sectionGap` margin above the grid that follows it, matching the margin already above the row itself; and, alongside the kind chips, a «Colaborare» / «Partnership» chip is offered whenever a partnered event is present among what the page shows (or the address already names the filter), narrows the listing (and the calendar's month/year view) to partnered events, and AND-combines with any kind chip pressed at the same time, its own state carried in the address as `?partner=1`.
66. The listing card of an event whose page shows a registration button carries the same button under its registration line. It uses the page's own component and words and goes to the same form or organizer's page: «Înscrie-te la eveniment», «Intră pe lista de așteptare», «Înscrie-te pe {provider}». The card shows no button wherever the page shows none: a full waiting list or no waiting list (§348), a window not open yet, a closed, cancelled or completed event, or no registration. The button is at least 44 px tall, and the listing does not scroll sideways at 320 px (2026-09-25, `DECISIONS.md` §409).
67. The card's registration line is bold where registration is open, about to open, full, or held by the organizer. While registration is open it ends with the free places out of the event's size, «· 7 locuri libere din 10» / «· 7 places left out of 10», or with «· Lista de așteptare» / «· Waiting list» once the places are gone. That number is the one the page shows beside its button. An uncapped event shows no number. A closed, cancelled or registration-free event keeps its quiet line (2026-09-25, `DECISIONS.md` §409).
68. An event's difficulty is one of five levels, in this order: VERY_EASY, EASY, MODERATE, HARD, VERY_HARD. In Romanian they read «Foarte ușor · Ușor · Mediu · Greu · Foarte greu», in English «Very easy · Easy · Moderate · Hard · Very hard». There is one ordered list in the domain; the database enum declares the same values in the same order, and any sort by difficulty follows the scale, never the alphabet. An event saved with EASY, MODERATE or HARD keeps its value (2026-09-25, `DECISIONS.md` §412).
69. The editor's difficulty choice offers «Nespecificat» and then the five levels in scale order, each with its gauge. Any of them can be saved, and a value outside the set is refused (2026-09-25, `DECISIONS.md` §412).
70. The difficulty pill's glyph is a gauge on every surface that draws it: the listing card, the event page's route row, the hero, the backoffice event cards and the editor's choice. The gauge is one half-dial `<svg>`, one icon wide. It has five segments; the segments up to the level are lit in the pill's own ink, the rest are at the theme's disabled opacity. Its needle points to the middle of the level's own segment, from left (very easy) to right (very hard). It is `aria-hidden`, and a Server Component draws it by name (`difficulty:VERY_EASY` … `difficulty:VERY_HARD`). This replaces §399's scale of weights (2026-09-25, `DECISIONS.md` §412).
71. The difficulty pill shows the level's word beside the gauge. A screen reader hears the word followed by the field's name («Mediu — Dificultate» / «Moderate — Difficulty») through `GlyphChip`'s visually-hidden `srSuffix`, for all five levels in both languages, and never through an `aria-label` (2026-09-25, `DECISIONS.md` §412).
72. The listing card's difficulty pill fits a 320 px phone without sideways scroll, and its width is measured in the e2e report. The «Mediu» pill measures 67.9 px, against 103.9 px with §399's weights; «Foarte greu» measures 98.3 px; the gauge is 18 px (2026-09-25, `DECISIONS.md` §412).
73. Given the event listing, when it renders, then its filters are one «Filtre» / «Filters» button, closed by default and at least 44 px tall, opening a native fold that holds a GET form of checkbox pills in named groups: type, surface, difficulty, distance (up to 5 km, 5–10, 10–21 with a half marathon up to 21.1 km inside it, over 21) and cost, and under «Altele» / «More» the flags «Colaborare», «Eveniment de noapte» and «Înscrieri deschise». Each option is a 44 px tall label (2026-09-25, `DECISIONS.md` §413).
74. Given the filters in the address, then boxes ticked in one group are either-or, and ticked groups must all match. An event that leaves a question unanswered matches no box of that group. The featured event is shown only when it matches, and is never shown lower down as a card (2026-09-25, `DECISIONS.md` §413).
75. Given the rows on view, then a box is offered only where ticking it would change what the page shows, or where the address already ticks it. A group needs at least two values that narrow the page before it is offered, and nothing to narrow with nothing ticked renders no button (2026-09-25, `DECISIONS.md` §413).
76. Given «Înscrieri deschise» ticked, then an event matches exactly when its own page shows a registration button right now: a free place or an uncapped event, a waiting list that still takes people, or the organizer's own form. It does not match when the event is full with no waiting list or a full waiting list, the window is not yet open or is closed, the event is cancelled or finished, or the event takes no registration. The free places come from the same cached availability the page reads, read only for an open internal event, and an event whose count cannot be read does not match (2026-09-25, `DECISIONS.md` §413).
77. Given an address, then the filter reads repeated parameters (the form's own shape) and comma-separated values alike, trimmed. It accepts every enum name in any case and with `-` for `_` (`?type=race,group-run`) and the distance in kilometres (`0-5`, `5-10`, `10-21`, `21+`, `21-`). A value outside its closed set is ignored, and `?type=RACE` and `?partner=1` mean what they meant before (2026-09-25, `DECISIONS.md` §413).
78. Given ticks in force, when the fold is closed, then the button counts them («Filtre (2)») and a row of links follows it. Each link removes its own tick («Scoate filtrul: Cursă»), with «Șterge filtrele» at the end. Nothing matching says «Niciun eveniment nu se potrivește filtrelor alese.» rather than «nothing is published» (2026-09-25, `DECISIONS.md` §413).
79. Given a browser with scripts off, when the listing or the calendar is opened, then the panel, the featured event, the list, the past section and the calendar's month are visible in the first HTML the server sends, with no streamed boundary and no loading shape. Opening the fold, ticking and pressing «Aplică» / «Apply» loads the narrowed page, and the calendar keeps its month or year and layout. With a script, each tick applies at once through a soft navigation to the same address, and the apply button is hidden (2026-09-25, `DECISIONS.md` §413).
80. Given the calendar, then it reads the same filters and offers the same panel, drawn from the whole period on view. Its month and year links keep every tick, and its last good copy is the unfiltered period, filtered after the read (2026-09-25, `DECISIONS.md` §413).
81. Given the past section of the listing, then it reads one cached window of the club's latest 60 past events, the same data-cache entry whatever is ticked. It narrows that window in memory by the same filters, shows at most twelve cards, and leaves out the event the page already leads with between seasons (2026-09-25, `DECISIONS.md` §413).
82. Criterion 12, amended: given the gallery, when it is asked for, then the parts that do not depend on its query render at once and the album grid is replaced, until it arrives, by a loading shape of the same box with `role="status"` and a name in the reader's language. The event listing and the calendar instead render once from their one cached read, with no loading shape and no streamed region. When the reader changes the month, the year, the layout or a filter with a script, the page stays on screen until the next one is ready, the calendar's controls do not move, and nothing on the page is blanked (2026-09-25, `DECISIONS.md` §413).
83. Given a listed event (a series by its next date) whose start is less than seven days ahead, when the listing renders and Open-Meteo answered for its place, then its card carries in its marks row a 24-pixel outlined pill with the forecast's glyph and the whole-degree temperature («14 °C»). Its accessible name is «Vremea la start: <kind>, <temperature>», and the rain, the wind and the details stay the page's. The featured event at the head of the listing carries a «Vremea» / «Weather» line among its facts: the kind, temperature, chance of rain and wind, and the credit. The listing credits «Prognoză: Open-Meteo» once, under the cards, as a link at least 44 pixels tall, only while a card carries a forecast. Nothing is shown beyond seven days or when the forecast is unavailable, the calendar's month view is unchanged, and nothing on the listing is wider than a 320-pixel phone (2026-09-25, `DECISIONS.md` §416).
84. Given a listing card whose summary holds a picture or a YouTube film, when it renders at any width, then every picture and film is shown whole on the card, whatever the length of the words. Only the words are clamped, to three lines counted across their paragraphs. A picture written before the first words stands above them, and every other picture or film stands under them in the order written. The event page and the hero render the summary whole and in its own order (2026-09-25, `DECISIONS.md` §417, amending §366).
85. Given the public registration form of an event with its own conditions, with JavaScript on, when it renders, then the required conditions checkbox is visible inside the read button's surface: the box and the button on one row, the box's middle within the button's height, both at least 44 by 44 pixels. Pressing the box or the button opens the conditions and does not tick the box. The panel's agree button stays disabled until the text has been scrolled to its end, or has nothing to scroll. Agreeing ticks the box. Once reached, the end stays reached for later openings. The text is measured again when its box resizes, and can be scrolled from the keyboard. A submission refused for another reason brings the box back ticked. With JavaScript off, the box is an ordinary required checkbox with a link (2026-09-26, `DECISIONS.md` §422, amending §195).
86. Given the public registration form, the ordinary one or the family link, with JavaScript on, when any control the browser would refuse is still empty or invalid, then a list above the send button names each one by the same short name the refusal summary uses, in the form's order. Each name is a link at least 44 pixels tall that opens any fold around its field, brings it into view and focuses it. The list updates as fields are filled or ticked, including a tick given from the conditions panel, and is gone once nothing is missing. A control in a disabled fieldset is never listed (2026-09-26, `DECISIONS.md` §422).
87. Given any required field on the registration form, when its label renders, then it carries exactly one asterisk: the one MUI draws for `required`. No catalogue label carries its own (2026-09-26, `DECISIONS.md` §422).
88. The listing's and the calendar's «Filtre» button and each checkbox option in its panel draw a small pill, at most 26 pixels tall, the size of the active-filter chips. The element a thumb hits (the `<summary>`, each `<label>`) stays at least 44 pixels tall (criterion 6). Proven by `tests/e2e/listing-filters.spec.ts` on both projects and `tests/unit/events/filter-chip-sx.test.ts`.
89. A listing card whose start is within seven days shows its weather as the last pill of the route's row, as tall as the pills beside it, with the sky's glyph and the degrees; a screen reader hears «Vremea la start: <sky>, <degrees>» (2026-09-26, `DECISIONS.md` §429).
90. When rain is likely at the start (a precipitation chance of 50% or more, or 0.5 mm or more forecast in the hour, unless the hour's glyph is snow, snow showers, frost or a storm), the card's weather pill shows an umbrella beside the sky's own glyph, never in its place, and says «ploaie probabilă» / "rain likely" after the degrees (2026-09-26, `DECISIONS.md` §429).
91. The featured hero's «Vremea» line and the event page's forecast show the same umbrella and words, placed right after the rain-chance phrase and before the wind; with no chance but an amount already falling, right after the temperature (2026-09-26, `DECISIONS.md` §429).
92. The listing has no Open-Meteo credit strip under its cards; the only Open-Meteo link in its main region is the featured hero's own, on its «Vremea» line, and only when the hero shows a forecast (2026-09-26, `DECISIONS.md` §429).
93. The site footer's «Despre club» fold carries «Prognoză: Open-Meteo» / "Forecast: Open-Meteo" as a link to Open-Meteo's site, opening in a new tab with rel="noopener noreferrer", visible once the fold is opened; the event page keeps its own credit beside its forecast (2026-09-26, `DECISIONS.md` §429).
94. Given a backoffice ActionForm whose Server Action call fails in transport — the fetch rejects in the browser's own fetch-failure words («Failed to fetch», «NetworkError…», «Load failed») or Next answers E394 for a response that is not a Server Action's — when the failure arrives, then nothing is thrown and nothing is sent again: the boxes keep what was typed, and a warning above the form offers one button, «Trimite pe calea simplă» / "Send the simple way", beside «Dacă ai primit deja mesajul că s-a salvat, nu apăsa — verifică pagina.» (2026-09-26, `DECISIONS.md` §436).
95. Given that warning, when the person presses «Trimite pe calea simplă», then the form as it stands, with the button first pressed, leaves as a plain multipart/form-data POST to the same page carrying the Server Action's no-JavaScript fields — taken from the form, or from the page's own HTML matched by the key the server stamps, and never sent when the match is uncertain. The 60-second cookie is set only by this press. If it cannot leave, the warning says nothing was sent and suggests another network (2026-09-26, `DECISIONS.md` §436).
96. Given a refusal, a redirect, a 404, a server error (anything with a digest), "Server Action not found" or any TypeError not in the browsers' fetch-failure words, when a backoffice action or render fails, then no simple-way button is offered and the error goes where it always went (2026-09-26, `DECISIONS.md` §436).
97. Given a plain backoffice form or a button with its own Server Action whose failure reaches the admin error boundary, when the guard remembered its press within the last ten seconds and the error is a fetch failure or E394, then the boundary offers the same button, sending only on the press. Otherwise — no recent press, or any other error, such as a TypeError from a render right after a press — the error is rethrown to `[locale]/error.tsx` with its reference number (§52). Sign-out, whose form lives in the admin layout, is not covered (2026-09-26, `DECISIONS.md` §436).
98. Given the simple way was pressed and the plain POST landed, when the next backoffice page renders, then a dismissable line says the simple way was tried — «Rețeaua de la birou a blocat cererea de salvare, așa că am încercat calea simplă. Salvarea a ajuns doar dacă vezi și mesajul de confirmare; …» — and never that the save arrived, which only the §384 confirmation says. The line clears its cookie, so a refresh shows nothing (2026-09-26, `DECISIONS.md` §436).
...
36. The listing card and its featured hero state, for an EXTERNAL-registration PAID event, that the fee is paid to the organizer rather than the club, matching the event page, the .ics description and the JSON-LD offer already fixed by the implementer; a screen reader is told this even though the card's visible pill keeps the closed set's own short word.
37. An event held with one or more partners shows them, on the event page, in a `<section id="partners">` — a native `<details>` fold whose `<summary>` carries the Handshake glyph, the club's "Împreună cu" / "Together with" lead and every partner's name — closed by default below the `sm` breakpoint and forced open from `sm` up; the §344/§352 partner cards render inside it unchanged, and the block sits after the page's other facts rather than as one of their rows.
38. Given a SCHEDULED event whose start (a race's start time when it has one) is less than seven days ahead, when its public page or preview is rendered and Open-Meteo answered, then the facts show a «Vremea» / «Weather» row, after the short facts and before the partners. The row has the forecast's glyph, its kind, the whole-degree temperature, the chance of rain and the wind, with the credit «Prognoză: Open-Meteo» linked to Open-Meteo's site. Given a start behind us or more than seven days away, an event not going ahead, or a failed request, then there is no row and no sentence about a missing forecast (2026-09-25, `DECISIONS.md` §402).
39. The forecast is asked on the server only, for the club's place `CLUB_COORDINATES`, at most once an hour through the data cache (the place is part of the cache key), with a three-second timeout. A failure is not cached, and the instance does not ask again for five minutes. Given a cached answer older than two hours (MAX_FORECAST_AGE_MS), when a page reads it, then Open-Meteo is asked again at once and that answer is shown. When that request fails, no row is drawn and the old answer is never shown as current (2026-09-25, `DECISIONS.md` §402).
40. An event's YouTube film is fetched once, server-side, and stored as the club's own poster (never a request to i.ytimg.com from a visitor's browser); the facade shows it as a real <img>, falls back to a plain dark rectangle when none is stored yet, and the iframe embed itself is never fetched until the visitor opens it (2026-09-25, `DECISIONS.md` §403).
41. The poster/play control/iframe is a native <details>/<summary> disclosure, rendered by a Server Component, so pressing play works with no JavaScript running at all; only the mute/volume bar is a client island, and it renders nothing (and talks to nothing) until its <details> ancestor is open (2026-09-25, `DECISIONS.md` §403).
42. Given a SCHEDULED event whose start (a race's start time when it has one) is less than seven days ahead, when its public page or preview is rendered and Open-Meteo answered, then the facts show a «Vremea» / «Weather» row, after the short facts and before the partners. The row has the forecast's glyph, its kind, the whole-degree temperature, the chance of rain and the wind. Under that first line come: one line of the start hour's details (feels like, precipitation in mm from 0.1, gusts, humidity, the UV index when it rounds above 0), each only when the hour has it; the start hour and the two hours after it as an ordered list named «Pe ore, de la start» / «Hour by hour, from the start», each cell with its hour, glyph, temperature and chance of rain, in one row at 320 pixels; and «Pentru locul evenimentului» / «For the event's place» (or «Pentru Brașov» / «For Brașov» when the club's place was read) before the credit «Prognoză: Open-Meteo», linked to Open-Meteo's site. Given a start behind us or more than seven days away, an event not going ahead, or a failed request, then there is no row and no sentence about a missing forecast (2026-09-25, `DECISIONS.md` §416).
43. The forecast is asked on the server only, for the event's own place, taking the first of these that gives a point: the coordinates the map link carries in its own address; the typed «Coordonate»; `CLUB_COORDINATES`. A short link (`maps.app.goo.gl`) is never followed, and a place to be announced reads the club's place. The place is rounded to three decimals, which is both the cache key and the request, and it is asked at most once an hour per rounded place through the data cache, with a three-second timeout. The same request asks for apparent_temperature, precipitation, wind_gusts_10m, relative_humidity_2m and uv_index; an answer without them is still a forecast, with no details. A failure is not cached, and the instance does not ask again for five minutes. Given a cached answer older than two hours (MAX_FORECAST_AGE_MS), when a page reads it, then Open-Meteo is asked again at once and that answer is shown. When that request fails, no row is drawn and the old answer is never shown as current (2026-09-25, `DECISIONS.md` §416).
44. The event editor's «Locul» card has one optional box, «Coordonate (lat, lng)» / «Coordinates (lat, lng)». It takes «45.6427, 25.5887» and the decimal comma («45,6427; 25,5887»). Given anything else that is not two numbers in range, then the save is refused naming the box. An empty box clears the pair, and a save that does not post the box leaves the stored pair unchanged. The database refuses half a pair or a pair out of range. Under the box, a saved event says which place the weather is read for: the map link's point, these coordinates, or the club's place because the link carries none (2026-09-25, `DECISIONS.md` §416).
45. Criterion 42 adds: when the rain-likely rule holds, an umbrella with «ploaie probabilă» / "rain likely" follows the chance-of-rain phrase on the event page's weather block (after the temperature when the hour has an amount but no chance).
...
27. A night event carries a headlamp pill on the listing card, the event page and the hero, «Alergare de noapte» / «Night run» on a group run and «Eveniment de noapte» / «Night event» on any other type. Its tooltip gives the day's sunset, and, when the start alone was not dark, the end's time and whether it is the event's end or the programme's last row, in both languages (2026-09-25, `DECISIONS.md` §394).
28. The calendar's month view and the `.ics` / Google Calendar description carry the night line with the sunset, worded as a run on a group run, computed from the same start, end and programme as the pill (2026-09-25, `DECISIONS.md` §394).
29. Every sentence about a night event names the occurrence's own start before that day's sunset: the pill's tooltip, the calendar entry, the `.ics` description, the reminder and the editor's automatic line. «Începe la 19:00, apusul la 19:00» / "Starts at 19:00, sunset at 19:00". A sunset time is never the only time shown (2026-09-25, `DECISIONS.md` §404).
30. When the automatic answer is night only because the span runs past dusk, the sentence also names that end, in the words of where it came from: «se termină la 20:40» / "ends at 20:40" for «Durata», and «ultimul punct din program la 20:15» / "the programme's last row at 20:15" for the day's last programme row (2026-09-25, `DECISIONS.md` §404).
31. When the sun alone made the call, no end is named and the start is after that day's sunset, the sentence says the start came after the sunset: «Începe la 20:00, după apusul de la 19:00» / "Starts at 20:00, after the 19:00 sunset" (2026-09-25, `DECISIONS.md` §404).
32. When the automatic answer is night and the start is before that day's sunrise, every sentence names the sunrise and never the evening's sunset. This covers the pill's tooltip, the calendar entry, the `.ics`, the reminder and the editor's automatic line. A 05:30 group run on 13 January 2027 at the club's place reads «Începe la 05:30, înainte de răsăritul de la 07:55» / "Starts at 05:30, before sunrise at 07:55" (2026-09-25, `DECISIONS.md` §404).
33. The shape of the sentence (plain, end, programme end, after sunset, before sunrise) is decided by one function, `nightShape` in `domain/night.ts`, which the tooltip, the calendar entry, the `.ics` and the reminder all use (2026-09-25, `DECISIONS.md` §404).
34. Given a night date, when the reader hovers or focuses the night pill on the listing card, the event page's route row or the hero, then its tooltip says only when the sun sets — «Soarele apune la {sunset}» / «The sun sets at {sunset}» — while the month view's line, the .ics description, the reminder and the editor's automatic line keep §404's full sentence (2026-09-25, `DECISIONS.md` §415).
35. Every sunrise, sunset, civil dawn and civil dusk the platform names is its computed instant rounded to the nearest minute. Each computed instant is within five seconds of the US Naval Observatory's published time for the same place and date, and the printed minute equals the Observatory's except where the instant lies within five seconds of a half minute (tests/unit/events/sun-reference.test.ts, §428).
...
12. While the privacy notice in force names {{participantListStates}} in every language, the public participant list shows beside each name one of three words — Confirmat, Înscris, în așteptarea confirmării (PENDING_DECLARATION, WAITLIST_OFFERED), Pe lista de așteptare (WAITLISTED, queue order, no position) — and lists the ticked pending and waiting after the confirmed rows.
13. The public list never shows a registration whose address is unconfirmed, a cancelled, expired or erased registration, a person who did not tick «Vreau să apar», or a TEST registration; the query selects a name, a club and a group, never the raw state.
...
9. Given Turnstile switched on with a wrong, missing or other widget's secret, or Cloudflare's internal error, when a person submits, then the check counts as not run and the person is not refused. A token Cloudflare rejects still refuses. `/api/health` degrades and `/admin/tasks` turns its row red only when the secret probe answers missing-input-secret or invalid-input-secret. A timeout, a 5xx or Cloudflare's internal-error reads as unreachable (2026-09-25, `DECISIONS.md` §420).
10. Criterion 3 is extended (§446). Given the same address posted to the public form for a different person (another name and another birth date than every registration on it), for a slip (the registered name with another birth date, or another name with a registered birth date), or for the same person again, when the responses are compared, then the redirect and every cookie set are byte-for-byte identical to a first registration's. Only the outbox differs.
...
5. Given a registration still waiting for its email confirmation, when its link lapses, then the verification token lapses with it. A click after the lapse, before or after the sweep, lapses the row instead of allocating it, and the page says the link has lapsed, never "confirmed" (2026-09-25, `DECISIONS.md` §420).
...
17. Given an event whose minimum age is N (`events.min_age`, 0 to 99, 14 by default and for every event created before the column), when a registration is submitted by any door (the public form, a staff entry or the desk's walk-in, a TEST row, or a restart), then a birth date under N on the event's day in its own time zone is refused as in criterion 11, and N = 0 refuses nobody for age. The public picker's `max` is the last birth date that reaches N on that day, or today when N = 0. The line before the first field states the minimum as its own sentence and, where a minor may enter, a second sentence naming the parent and their consent, in words: "Vârsta minimă: {N}." for 1 to 17, followed by "Sub 18 ani, înscrierea se face de un părinte, cu acordul acestuia."; "Vârsta minimă: {N}." alone for 18 or more; and only the parent sentence for 0 — English mirrors it: "Minimum age: {N}." / "Under 18, a parent registers the runner, with their consent." The birth date's help, the public refusal and the staff refusal name N. The staff form shows "N+" beside each event's name. The number is written with Romanian's "de" ("20 de ani", "101 ani") by `yearsPhrase` (2026-09-25, `DECISIONS.md` §410, amending §329 and §321). Verification: unit and integration `registrations/minimum-age.test.ts`; e2e `registration-form.spec.ts`.
18. On the family link, for a person who is an adult on the event's date, the health note, the socials, the public-list tick and the first-person fitness statement are dropped on the server whatever was posted, and an acknowledgement is asked in place of the statement. A minor's form is unchanged, and the eighteenth birthday is decided at one instant for both rules (2026-09-25, `DECISIONS.md` §421).
19. Given a public submission missing any of first name, last name, date of birth, sex, nationality, phone, emergency contact name or emergency contact phone, when it is posted, then it is rejected and no registration row is created (city is optional, §322).
20. Nationality is required on the public form, in the required half next to sex, and starts on Romania (RO); a staff entry leaves it optional, and a row without one stays null. The city is optional, offered under «Orașul tău — opțional» with a sentence saying why it is asked. A blank city is stored as null. The sex and emergency-contact fields explain why they are asked (§322, §432).
...
10. Given an address holding a closed registration with data still to withdraw, when "Înscrierile mele" lists it, then the card names whose registration it is (2026-09-25, `DECISIONS.md` §420).
...
8. Given an active registration on an address, when the public form is sent from that address for a person whose folded name (whitespace, case and diacritics ignored) and birth date both differ from every active registration on it, then no registration is created, the posted form is kept without the address, without the anti-bot fields and — for an adult — without the health note, the socials, the list tick and the fitness statement, until the club's email-link window closes; and the address receives one REGISTER_ANOTHER_PERSON email listing its own active registrations as first name and initial, naming the person with the birth date, and carrying one single-use hashed link to /registrations/family/[token] (§446).
9. Given that link, when its page is opened, then nothing is written and the token is not spent; when its button is pressed (with the acknowledgement ticked for an adult), then the person is registered under the same participant through the ordinary allocator, under the event's lock and the club's limit per address, the address is confirmed without a second confirmation link, the registration is PENDING_DECLARATION with its declaration email (or WAITLISTED with its message), and the kept form is deleted; a refusal (already registered, at the limit, the acknowledgement missing, the waiting list full) leaves the link working (§446).
10. Given an active registration on an address, when the public form is sent from it with only one of the name and the birth date matching a registration, then nothing is registered or kept, and the existing registration's message is re-sent with the sentence on how to register somebody else — in the inbox only (§446).
11. Given a kept form not confirmed before its window closes, when the registration maintenance job runs, then the form is deleted with its data and its link opens the generic invalid-link notice; and a REGISTER_ANOTHER_PERSON message rendered after its kept form is gone carries no button and says the request has lapsed and how to send the form again, in both languages (§446).
12. Given several confirmation links of one address pressed at the same moment with one slot left under the club's limit, when they commit, then exactly one registration is created and the others are refused with the limit's marker, their links still working (tests/concurrency/family.test.ts, §446).
...
9. The event's own participation-window deadline (opens/deadline days before start, default 7/2) may be set to a deadline of zero, meaning the confirmation window runs until the start rather than expiring early; every reader-facing sentence (public steps, editor help, 'Înscrierile mele', emails) must say 'until/la start' and never a hardcoded day count, and must not read as if signing stops at the reminder email's date.
10. Given an event with a participation window, when a verified participant restarts a registration on the public form, then the hold runs to the window's deadline, not the thirty-minute hold (2026-09-25, `DECISIONS.md` §420).
...
33. On a group run that offers its surface's approved declaration, the page shows a 44-px "Semnează declarația pe propria răspundere" button under the route. The signing page (/ro/evenimente/[slug]/declaratie, /en/events/[slug]/declaration) reuses the race declaration's text panel, identity-document boxes, signature box, consent, refusal summary and PDF renderer, and is bound to the version's id and hash. A signature writes one group_run_declarations row and never a registration. It queues the signer's copy with the whole PDF and, when a declarations mailbox is set, the club's copy with the identity document masked. It is guarded by honeypot, timing, Turnstile and a per-address throttle (tests/integration/group-run-declarations/sign.test.ts, tests/e2e/group-run-declaration.spec.ts).
34. The retention sweep deletes a group-run declaration and the outbox rows about it seven days after the run's start, and keeps one whose run is less than seven days gone (tests/integration/group-run-declarations/sign.test.ts).
35. The platform's declaration template carries no waiver. Its liability paragraph is informed acceptance that says it is no waiver (Civil Code art. 1355(4)). The organiser disclaims only what is solely the runner's or a third party's doing, with 'în limitele permise de lege'. The bullets add: health 'to the best of my knowledge' with the cardiac risk; heat and cold; traffic duties; slow help; no alcohol or drugs; first aid and being stopped. There are 14 bullets. The paragraph on accompanying minors disclaims nothing (2026-09-25, DECISIONS.md §418). Verification: unit legal-documents/declaration-risks.test.ts.
36. The signing box accepts the declaration 'including, expressly, the limitation of the organiser's liability to the extent the law allows'. An adult's declaration page names the person who must sign personally. The signed PDF says the link went to the registration's email address. While any entry of the event's bundle carries an identity document, every page says to delete the file within seven days of the event (2026-09-25, DECISIONS.md §418). Verification: unit registrations/declaration-signing-words.test.ts.
37. The allowance forecast charges each hidden-copy address one message per copied participant message: four per completed registration, since the address-confirmation link is never copied. The panel's "of those, N are copies" line uses the same figure (2026-09-25, `DECISIONS.md` §419).
38. A signed declaration lands on its outcome page with a toast that matches it: confirmed ("Declarație semnată: înscrierea ta e confirmată.") or on the waiting list ("Declarație semnată: ești pe lista de așteptare."), in the reader's language, shown once. Without JavaScript the page's banner is the whole answer.
39. Given a group run that offers its surface's declaration and whose event has a minimum age above zero, when the signing page renders, then it asks for «Data nașterii» / "Date of birth", bounded by the latest date still that old on the run's day. When a signer's date is under the minimum on the run's day in the run's zone, the press is refused. The focusable summary names the birth date with «Vârsta minimă pentru această alergare este {age}, împliniți în ziua alergării.», and no row, message or throttle count is written. A missing or unreadable date is named with the other wrong boxes. A run with no minimum asks for no date. The date is never stored. The comparison is the registration's own (`isUnderMinimumAge`). (2026-09-26, `DECISIONS.md` §440, amending §393). Verification: integration `group-run-declarations/sign.test.ts`; unit `group-run-declarations/minimum-age.test.ts`.
40. Given the platform's two group-run templates, in both languages, then each carries, right after its opening, a sentence naming `{{minimumAge}}`. It is filled from the event with its unit («16 ani», «20 de ani», "16 years") on the signing page and in the signed PDF. On an event with minimum age 0 the whole sentence is left out, and with no value at all it shows the dotted blank (2026-09-26, `DECISIONS.md` §440). Verification: unit `group-run-declarations/minimum-age.test.ts`, `legal-documents/group-run-declaration.test.ts`; e2e `group-run-declaration.spec.ts`.
...
6. Given an event whose registration has closed or whose start has passed, when a place is freed, then nobody on the waiting list is emailed an offer, because an offer made then would expire the moment it is made. The waiting list stays WAITLISTED for the desk, an offer made before the close lapses at the close and is not handed on, and the `/admin/emails` forecast promises no such offer (2026-09-25, `DECISIONS.md` §420).
...
5. Given a lapsed declaration hold after the registration close while somebody waits, when the maintenance job runs, then the hold is released to the queue as §160 says, its provisional number with it, and the desk can promote the waiting runner into the freed place instead of being told the event is full (2026-09-25, `DECISIONS.md` §420).
...
9. Signing binds the version read: a different version in force at the press is refused and nothing is written. The text is signed in the page's language, and the PDF and the email follow it (2026-09-25, `DECISIONS.md` §393).
10. Signing writes one `group_run_declarations` row and no participant, registration or place. It queues the signer's copy with the whole PDF and, when the club has a declarations mailbox, the archive copy with the identity document masked, in the same transaction. Neither message carries a token or an action link (2026-09-25, `DECISIONS.md` §393).
11. Every wrong box is named at once in a focusable summary, and nothing is recorded. The honeypot and the timing check answer as a signature and write nothing. A throttle keyed on the hashed canonical address answers "limited". A posted event or document id that is not a uuid gets the form's refusal before any query (2026-09-25, `DECISIONS.md` §393).
12. Given a family on one address (§389), when the form is sent once per child and each time emails a "register another person" link, then every one of those links stays usable for the club's email-link window, not only the newest. Each link registers at most one person and is single use. The address's limit is counted under the event's lock whenever a link is used, so several live links never add a registration the limit refuses (2026-09-25, `DECISIONS.md` §420).
13. Given a live declaration link on a registration that has since moved on (cancelled, expired or confirmed), when it is opened, then the page says where the registration stands instead of showing a form whose press fails. A signature that loses a race on a still-signable state says it could not be recorded (2026-09-25, `DECISIONS.md` §420).
...
13. A spare worn by an erased registration counts as taken: it is never printed blank again or suggested to anyone else (2026-09-26, `DECISIONS.md` §444).
...
11. Given a desk entry whose duplicate check found the address empty, when a public submission on the same address lands before the entry takes the event's lock, then the entry is refused under the lock with "this address already has a registration for this event", rolled back whole, and confirms nobody: no re-send, no audit row in the staff member's name, and the other runner stays as the public form left them. Proven on two real connections in `tests/concurrency/desk-race.test.ts` (2026-09-25, `DECISIONS.md` §420).
12. Given an event with reserved spares, when the desk renders a walk-in (a real runner with no number, settled or provisional), then the next free spare is suggested beside «Confirmă aici», in the number given by hand and in the walk-in form's box. When every spare has been given, the box is empty and the row says so. Given an online runner holding a provisional number, then no spare is suggested and the confirmation adopts their own number (2026-09-26, `DECISIONS.md` §444).
13. Given a number handed at the desk with the paper or typed by hand, then it is a whole number from 1 to 99999, refused when another registration of the event wears or holds it (settled, provisional or erased), and checked again under the event lock. When the number is a desk spare it becomes the runner's final number and is marked printed at once. When a walk-in's handed spare went to somebody else in the meantime, the entry stands unconfirmed and the desk is told why (2026-09-26, `DECISIONS.md` §444).
...
12. A print of 1 to 50 spares reserves the numbers after the highest number any registration holds (settled, provisional or erased) on a first print, and the next free numbers after the reservation on a second. A number an extension steps over stays its holder's and is never a spare. The reservation is at most 500 numbers and within five digits (2026-09-26, `DECISIONS.md` §444).
13. No automatic draw (provisional, final or at close) ever hands out a reserved spare number. A spare reaches a runner only when a staff member hands it at the desk (2026-09-26, `DECISIONS.md` §444).
14. The desk's «Confirmă aici» offers the next free spare only to a walk-in: a real registration entered by staff (or holding no number at all) with no settled number and no printed bib. A registration made online keeps its provisional number, and the confirmation adopts it (2026-09-26, `DECISIONS.md` §444).
15. `confirmRegistrationByStaff` refuses a handed number (VALIDATION_ERROR on `bibNumber`, nothing written) for a registration that is not such a walk-in. It refuses a number someone else holds, settled or provisional, with CONFLICT on `bibNumber`. It checks both again under the event lock (2026-09-26, `DECISIONS.md` §444).
16. A spare handed at the desk, or typed by hand, is marked printed, so the unprinted sheet never prints it a second time and a cancellation lists it among the existing bibs. When every spare is given, the desk says so in words (2026-09-26, `DECISIONS.md` §444).
17. A walk-in whose handed spare was taken between the desk's check and the confirmation stays entered and unconfirmed, and the desk is told to confirm them with another spare. A walk-in the allocator puts on the waiting list takes no number, and the spare stays free (2026-09-26, `DECISIONS.md` §444).
...
22. Given a lapsed declaration hold released by the sweep, then its provisional race number is released like every other sweep's (and migration `0080` clears the rows left set). A desk confirmation after the numbers settle then gets a free final number, never a duplicate-key error (2026-09-25, `DECISIONS.md` §420).
23. Given registration closes, when the numbers settle, then only confirmed, held and offered places get a final number and BIB_ASSIGNED. An unconfirmed address gives its provisional number back and gets none (2026-09-25, `DECISIONS.md` §420).
24. Given a waiting-list offer, from the queue or the desk's promotion, then it draws a provisional number under the event's lock like every other place. After the close, the desk's promotion adopts that number as the final one (2026-09-25, `DECISIONS.md` §420).
25. Given an event that takes registrations here, when an Administrator types how many spares (1 to 50) and presses «Tipărește», then, asked first with the first number named, the event's reservation of desk spares is made under the event-row lock. The first print takes the numbers right after the highest number anybody has, settled, provisional or erased. A later print extends the reservation after its end, stepping over any number somebody holds. A start that moved since the question is refused and nothing is written. The reservation is stored as a start and a count, both or neither, at most 500 numbers and within five digits (checked at the database), and audited with the range, never a name (2026-09-26, `DECISIONS.md` §444).
26. Given an event with reserved desk spares, when a number is drawn automatically, then it is never a reserved number. This covers the provisional number at submission, the recompaction at the close, a confirmation after it, the batch and the free-number suggestions (2026-09-26, `DECISIONS.md` §444).
27. Given a print whose extension reached numbers online runners already held, then each such number stays its runner's for good. The close keeps a provisional number inside the reservation as that runner's final number and never moves them out of it. The print records the numbers it stepped over, and none of them is ever counted, suggested or printed as a spare, even after it is released. No number is worn twice (2026-09-26, `DECISIONS.md` §444).
28. Given reserved spares, when the sheet is requested with `spares=1`, optionally within `from`/`to`, then it prints the free spares only, each bib with an empty name line and «înscris la fața locului» / "on-the-spot entry" under it, on the sheet and in the preview picture. Given the redirect after a print carries a numeric `from`/`to`, then the event page shows a banner naming that range with a link to its blank sheet; a range that is not two numbers of one to five digits, lowest first, shows no banner (2026-09-26, `DECISIONS.md` §444).
...
8. A create or a save that posts a difficulty (EASY/MODERATE/HARD) stores it and reads it back unchanged; `eventColumnsFrom` is the one place create and save both write the `events` columns through, and every field it accepts must actually be forwarded (2026-09-25, `DECISIONS.md` §398).
9. The event editor lays out its cards in the public page's order, on the create page and the editor alike, from one list of the page's sections (`PAGE_SECTIONS`) that a test holds to the page's own render order. Each card is headed by its section's number and whether the page shows it («N · Nume — apare pe pagină» / «gol, nu apare pe pagină»). The share links are named as automatic where the page draws them. The status, promotion and page address follow under «Nu apar pe pagină» (2026-09-25, `DECISIONS.md` §406)
10. Under Publicare and Recurență, a map lists the page's sections as chips in page order, each with its number, glyph, name and a filled or empty dot. Each chip is a link at least 44 px tall to its card, and opens it. The map is the same on create and edit and wraps on a phone (2026-09-25, `DECISIONS.md` §406)
11. Every closed card that holds a box publication needs names the missing boxes per language («lipsesc: Titlu (RO, EN) · Rezumat (RO)» or «complet») behind a warning glyph, its map chip is marked the same, and its language tabs count what is missing («Română · 2 obligatorii lipsă», «English · complet»). All of it comes from the same check as the Publicare list: over the boxes as typed, and over the saved event on the editor (2026-09-25, `DECISIONS.md` §406)
12. «Creează și publică» and the editor's «Publică» keep their full look on every event. Pressed while a box publication needs is empty (on the editor: empty in what is saved), they post nothing and open a focusable summary naming each missing box per language, bringing the first card that lacks one into view. The browser's refusal of an invalid box and the server's publication guard are unchanged (2026-09-25, `DECISIONS.md` §406)
13. «Creează și publică» is offered the same way on the create page for every event type and with or without a series. It is gated only by the type-agnostic publication check (2026-09-26, `DECISIONS.md` §423).
14. The create page's status card, inside «Ce fel de eveniment», is the editor's select: «Programat» by default, «Anulat» and «Încheiat»; no hidden status is posted beside it (2026-09-26, `DECISIONS.md` §448).
15. Creating an event as «Anulat» requires the cancellation's reason in both languages, refused on each empty box, and is allowed only to a role that may save the event row; the create records the `event.cancelled` audit row (who, why, told nobody) and queues no email (2026-09-26, `DECISIONS.md` §448).
16. Creating an event as «Încheiat» is refused on the status select while its start is still ahead (2026-09-26, `DECISIONS.md` §448).
17. An event created cancelled may be published, in the same press («Creează și publică») or later, and its public page says it is cancelled (2026-09-26, `DECISIONS.md` §448).
...
66. The route card offers "Declarație opțională pe propria răspundere" only for a group run. It ticks itself when the surface becomes Trail and stays off for Asfalt. It is disabled with a line naming the text to approve when the club has no approved text of that kind. A save keeps it only on a group run on asphalt or trail, the series scope carries it, and copies keep it (tests/integration/cms/group-run-declaration-flag.test.ts, tests/e2e/group-run-declaration.spec.ts).
67. An event's night status is the organizer's override when set («Da» true, «Nu» false), else computed automatically: night when any part of the span from the occurrence's start to its end is at or after civil dusk or before civil dawn of the start's own day at the club's place (`CLUB_COORDINATES`), including a span that contains that day's civil dusk or that ends on a later day on the event's wall clock. The end is the event's own `endsAt`, else the latest timed programme row of the occurrence's own date, else the start alone (2026-09-25, `DECISIONS.md` §394).
68. Each date of a series answers for its own sunset; a series' row on the listing shows its next date's answer (2026-09-25, `DECISIONS.md` §394).
69. The editor's «Traseul» card offers Automat (după apus) / Da / Nu, Automat by default. Under the choices it shows the automatic answer for the date, time, duration and programme currently in the form, by the server's own rule. When the end is the reason, it names the end's time and its source. A series adds a sentence saying each date follows its own sunset. The closed card says «de noapte (automat)», «de zi (automat)», «de noapte» or «de zi» (2026-09-25, `DECISIONS.md` §394).
70. Migration `0076_night_override` makes `events.headlamp_required` nullable with no default and turns every `false` into NULL (Automat), keeping `true` as «Da». Expand only (2026-09-25, `DECISIONS.md` §394).
71. The 'Publică datele noi automat' switch carries the renew (robot) glyph on every one of its lines — the create page's own switch and both of RecurrenceSeriesPanel's mentions — decorative and aria-hidden, drawn from the shared ACTION_ICONS.renew registry entry (2026-09-25, `DECISIONS.md` §398).
72. The backoffice's time box is the platform's native type="time" input, always posting HH:mm on the 24-hour clock; Chromium and Edge's own clock indicator stays visible and clickable, opening that browser's own time popup, rather than being hidden behind a duplicate AccessTimeIcon adornment (2026-09-25, `DECISIONS.md` §400).
73. Given a user who may not edit an event's text, when they save it, then the save is refused before any poster is fetched or stored on their behalf (2026-09-25, `DECISIONS.md` §403).
74. The event reminder's night line uses the same five shapes in each language half, «Alergare de noapte» on a group run and «Eveniment de noapte» otherwise, and ends with «Ia o frontală.» / "Bring a headlamp." No other message carries it (2026-09-25, `DECISIONS.md` §404).
75. The programme's rows in the editor are one grid per row, laid out by the list's own width: Data · Ora · Până la · Unde · delete, then «Ce (română)» · «Ce (engleză)» side by side; «Unde» moves to its own line when the list is narrow, and on a phone every box is stacked in reading order with delete last. The explanation is the compact «i» help fold, closed by default, and it holds the sentence about registered people. A row starts on the event's start date: the spare line opens on it, or takes it the moment it is first typed on the create page, and a new row opens on it. When the start date moves, dated rows move by the same number of days and undated rows take the new date. A row whose only box is its date is dropped on save, never refused (2026-09-25, `DECISIONS.md` §405).
76. The event editor and the create page lay out the cards that write the public page in the page's own order — the type, the title and summary, the description, the date, the place, the course, the cost, registration, the partners, (the share links, named as automatic, with no card), links and files, the programme, the rules, the film, the public participant list — then the cards that are not sections of the page (status, promotion, the page address); a test reads the public page and fails when either order parts from the list (2026-09-25, `DECISIONS.md` §406).
77. Each such card is headed by its number on the page, 1 to 14, and whether the page shows it for this event — «7 · Cost — apare pe pagină», «13 · Filmul — gol, nu apare pe pagină» — read from the stored columns with the rules the page's components follow, the night pill included for the course (2026-09-25, `DECISIONS.md` §406).
78. The cost is a card of its own, «Cost», placed where the page draws its cost row: the cost select, the amount and link, and the club's discount note; its closed line says the cost; a new event starts free; for a reader without settings rights it is its heading and its line, opening only onto the discount note on an EXTERNAL + PAID event (2026-09-25, `DECISIONS.md` §406).
79. A map under Publicare and Recurență lists the page's sections in order as chips of at least 44 px, each with its number, glyph and a filled or empty dot, each opening its card; the share links are named as automatic (2026-09-25, `DECISIONS.md` §406).
80. Every card that holds a box publication needs says on its closed line what is still missing, per field and language, or «complet», and its language tabs count the missing boxes («Română · 2 obligatorii lipsă») — from the server's first paint and then as typed — all from `missingForPublish`, the one check the Publicare list and «Publică» run (2026-09-25, `DECISIONS.md` §406).
81. «Creează și publică» and «Publică» are always drawn at full look; pressed while something is missing, they post nothing and open a focusable summary naming each gap by card and language, bringing up the first card (2026-09-25, `DECISIONS.md` §406).
82. The editor's «Fereastra de confirmare» card accepts 0 as the deadline and labels it "(0 = la start)". It then shows "Cerută cu N zile înainte, termen la start" on its closed line and "… termen la start — locul nu expiră înainte de start" on its dates line, never "0 zile". With the first number 0, or not above the deadline, it shows "Fără fereastră" on both. Its help says in four sentences, in both languages, when a runner can confirm (2026-09-25, `DECISIONS.md` §407).
83. With at least one real registration (test rows excluded), the event editor shows how many are registered exactly once. It is one line under the page map, with a tooltip naming the cards a change reaches and, for a role that may read registrations, a 44-pixel link to the event's registrations list. Those cards keep their amber outline and carry no count on their heading or in their sentences. With none registered, and on the create page, there is no line and no outline (2026-09-25, `DECISIONS.md` §408).
84. The event editor asks the duration as two whole-number boxes, hours (0–168) and minutes (0–59), and saves their sum in minutes as the event's duration; both empty is no duration; a fraction, a negative, sixty minutes or more, zero in total or more than a week is refused and the refusal names the hours box; a saved duration is shown back split into the two boxes and in the closed card line as «H h M min».
85. Every backoffice time-of-day box shows and posts `HH:mm` on the 24-hour clock. It is a text box, never the browser's `type="time"` control, so no browser locale can draw it as AM/PM, with JavaScript or without (2026-09-26, `DECISIONS.md` §439).
86. The time box keeps every keystroke as typed: «19:00» typed key by key reads «19:00», and no character is added or removed while typing (2026-09-26, `DECISIONS.md` §439).
87. On leaving the box or pressing Enter in it, «1900», «19.00», «19,30», «19h30», «19 30», «930», «9:30», «7» and «19» become `HH:mm`. «24:00», «19:60», «7pm», «07:00 AM» and «abc» are left as typed, for the box's `pattern` and the server's `isTimeValue` to refuse (2026-09-26, `DECISIONS.md` §439).
88. No 12-hour value is read or shown anywhere in the backoffice's time fields, and the registrations workbook writes its times on the 24-hour clock (2026-09-26, `DECISIONS.md` §439).
89. Amends criterion 29: given the editor or the create page of a group run, when it renders, then the «Traseul» card offers «Vârsta minimă (ani)» beside the optional declaration, whose help says the number is shown in the declaration and enforced at signing. It is prefilled with the event's value or 14, has the same 0–99 bounds, and is saved as the event's `min_age`. The race's registration box is not read for a group run (2026-09-26, `DECISIONS.md` §440).
90. Amends criterion 76. The cards that are not sections of the page are the promotion and the page address. The status is inside the type's card (2026-09-26, `DECISIONS.md` §448).
91. Given the create page or the editor of an event, when «Regulamentul» renders, then it holds a named card «Declarația pe propria răspundere» (`#box-declaration`) after its language tabs. For a group run the card shows the surface read from «Traseul», the offer checkbox `event.offersGroupRunDeclaration`, and the approved text in force for that surface with its version, or the line that none is approved. For a type that takes registrations and registers on the site, it holds the `event.declarationDocumentId` select among approved versions and names the newest. For any other mode it says no declaration is asked. «Traseul» and «Condiții de participare» post neither field. «Regulamentul»'s closed line adds the declaration part, and the box and the card open by themselves while a saved event registering on the site has no declaration. A refusal of either field names «Regulamentul › Declarația pe propria răspundere › …» and opens both folds. Verification: unit `content/editor-declaration-card.test.ts`, `content/box-summaries.test.ts`; e2e `group-run-declaration.spec.ts`, `forms-keep-values.spec.ts` (2026-09-26, `DECISIONS.md` §448).
...
19. A new event's cost select preselects "Gratuit" (`initialCostTypeOf(null) === "FREE"`), and a save that never opens the cost box still stores FREE, read as "Gratuit"/"Free" on the public page in both locales; proven under a real production build and a migrated database, not a reused stale server (2026-09-25, `DECISIONS.md` §398).
20. Given a YouTube film on an event (`events.video_url`) or as a `youtube` block in any of the five rich texts (body, rules, schedule, route description, rich summary) in either language, when it is saved, then the server fetches the video's thumbnail once, before any transaction opens, re-encodes it and stores it as the club's own media asset keyed by the video id, and records its address on the event (`events.video_poster_url`, migration `0077_video_poster`) or on the block. When the page renders, the closed disclosure shows that stored poster and fetches nothing from YouTube (2026-09-25, `DECISIONS.md` §403).
21. Given the poster fetch fails or never answers, when the event or page is saved, then the save still succeeds without a poster and no poster fetch runs inside a transaction, in createEvent, createEventAndPublish, saveEventFields, saveEventAndTranslations, saveEventTranslation, createPage or savePage (2026-09-25, `DECISIONS.md` §403).
22. Given a picture in the text or a film's poster uploaded in the editor, when it is stored, then it is stored at the chosen quality exactly as an album photo is (BR-REQ-054-01 criterion 12). The toolbar's picture control opens the choice before the file dialog, and a picture pasted or dropped in uses the same remembered choice. A film's panel carries the same choice beside «Alege un thumbnail», and the poster's size is kept on the film as `posterWidth` / `posterHeight`. The editor states what each upload became. The public page gives a picture with a ladder a `srcset` and a `sizes` from its column (the event page, a standing page's measure or a card), its share and its crop, and gives a club poster the same over its 16∶9 box. A picture or poster from before the ladder, and YouTube's own thumbnail, keep their one `src`. Verified by unit `media/ladder.test.ts`, `events/video-facade.test.ts` and e2e `image-quality.spec.ts`, `route-description.spec.ts` (2026-09-25, `DECISIONS.md` §414).
23. Given a converted older picture (BR-REQ-054-01 criterion 14), when the press commits, then its address is rewritten to the laddered prefix in every text the reference check reads: a page's body, an event translation's description, summary, rules, programme and route description, and the event's film poster. Every rewritten row's version goes up by one, and a translation without the picture is untouched. The public page then serves the picture with `srcset` naming every rung and the same master, every candidate answers 200, and the old address still answers with the same file (§430). Verification: integration `cms/older-pictures.test.ts`; e2e `older-pictures.spec.ts`.
24. Criterion 22 (extended from two levels to four): every picture upload in the rich-text editor and the film poster panel offers the same four levels, with the same limits as BR-REQ-054-01 criterion 12. The help text says what each level keeps, in both languages.
25. New criterion: before a picture is uploaded, the editor, the poster panel and the album uploader show the chosen file's pixels and weight. When the browser re-encodes the file, they also say what is sent: smaller when the pixels change, lighter when only the weight does. After upload they state the stored size, the quality, the file count and the widest smaller copy's width and weight. The pictures list shows that copy's width for each picture stored with a ladder.
...
12. Given a photo uploaded to an album, when it is stored, then it is stored at the quality chosen beside the upload button: «Normală» (the default, and what a request without a choice gets) keeps at most 2400 px on the long side; «Înaltă» keeps at most 4000 px, lossy at quality 90 or near-lossless when that costs at most twice as much, which is how posters and lettering come out. A value that is neither is refused with nothing written. The browser sends at most 3000 px for «Normală» and 4000 px for «Înaltă», sends a file that fits as it is, and keeps every request under 4 MB. Besides the master and the thumbnail, the photo is stored at every width of 480, 640, 960, 1280, 1600, 1920 and 2400 that is narrower than 0.9 of the master, all of them removed with the photo. The album page's tiles and the albums page's covers offer those widths as `srcset` with the drawn width as `sizes`, so a 390-px phone at 3× takes a rung and never an enlarged thumbnail. A photo stored before the ladder has no `srcset` and keeps its thumbnail. The choice is remembered for the tab, in the page's memory when the browser refuses the store, and after the upload the page states the last photo's size, quality, bytes and number of files. Verified by unit `media/images.test.ts`, `media/ladder.test.ts`, `media/browser-shrink.test.ts`, `media/image-quality-choice.test.ts`, integration `cms/image-quality.test.ts` and e2e `gallery.spec.ts` (2026-09-25, `DECISIONS.md` §414).
13. Every event page, whether a race or a group run, carries the photographs notice from the counsel review's amendment, word for word in both languages, with a way to ask for a photo to be taken down and a link to the privacy notice. The gallery's upload help ends with the upload rule, word for word, in both languages (2026-09-25, `DECISIONS.md` §421).
14. Criterion 14 — a press of the older-pictures button that fails a picture (missing master, undecodable master, a store refusal, or a transient per-picture exception) logs which asset id failed and records up to `perPress` of those ids on the audit row's metadata as `failedIds`, alongside the existing `converted`/`failed`/`left` counts (2026-09-26, `DECISIONS.md` §430).
15. Criterion 14 — the task board's failed-picture copy, in both Romanian and English, states that the file is usually missing from storage or not a picture, and hedges that if the picture is still failing after one more press it should be re-uploaded where it is used — it no longer asserts unconditionally that pressing again never helps (2026-09-26, `DECISIONS.md` §430).
16. An album may be linked to an event or be free (no event). The public gallery lists both kinds in one list, newest first by the day taken. An event album's card names its event only while that event is published in the reader's locale; a free album's card names no event.
17. The backoffice album list groups albums into event albums and free albums, each newest first, and each group says when it is empty.
18. Criterion 12 (extended from two levels to four): every album photo upload offers «Calitate: Minimă / Medie (recomandat) / Mare / Originală», with Medie as the default. Minimă keeps a master of at most 1280 px, Medie 2400, Mare 4000, and Originală the file's own pixels up to 6000. Nothing is enlarged. The 3200 rung exists only under a master wider than 4000, and a stored master of 4000 px or less never has a 3200 file in its srcset.
...
12. On a draft event's editor, a role that may publish is offered «Publică» beside «Trimite spre verificare». One press walks DRAFT → IN_REVIEW → PUBLISHED under the same guard and version check as «Creează și publică». A refused publication leaves the event a draft, and a role that may not publish is refused before anything moves (2026-09-26, `DECISIONS.md` §423).
...
9. on an EXTERNAL-registration, PAID event, offers.url is the organizer's own externalRegistrationUrl rather than the club's cost_url — the place where the offered thing is actually taken — and isAccessibleForFree stays false. The .ics description's cost line carries the same «Cost: <amount>, la organizator» wording, with the discount note appended when the club stated one (2026-09-25, DECISIONS.md §395).
...
2. on an EXTERNAL-registration, PAID event, the cost text reads «Cu taxă, la organizator» / «Paid, to the organizer» with the amount — never the wording an INTERNAL paid event uses, which would read as the club's own fee — and the club's discount note, when stated, follows it in the reader's language, both languages or neither (2026-09-25, DECISIONS.md §395).
...
20. Two optional legal documents exist beside the three a registration rests on: the group run's self-declaration for asphalt and for trail. They are approved, versioned and deleted like the others, and gate no registration: an approved one stands in for none of the three, and its absence stops none of them (2026-09-25, `DECISIONS.md` §393).
21. A group run's self-declaration is refused while no approved privacy notice is in force in the signing language. Nothing is written and nothing is queued. The run's page shows no offer and the signing page shows no form meanwhile (2026-09-25, `DECISIONS.md` §393).
22. A signature counts as reliance on its version: deleting that version and editing it are refused. Once the retention sweep has removed its last signature, a superseded version may be deleted again (2026-09-25, `DECISIONS.md` §393).
23. A group run's declarations and their outbox messages are deleted by the retention sweep 7 days after the run's start. The window is one constant, and the run's page, the signing page, the backoffice fold and both emails state it through that value, never as typed words (2026-09-25, `DECISIONS.md` §393).
24. The platform's terms template says: registration on the site is free, and on another organiser's event the club only publishes. §3, §4, §5 and §10 are accepted expressly, by a separate box. One address may register others with their agreement, and each person signs their own declaration; nobody signs for another adult. The courts are the ones competent under the Code of Civil Procedure, with a consumer's domicile as a choice, never the club's seat. The template points to ANSPDCP, not SAL-ANPC (2026-09-25, DECISIONS.md §418). Verification: unit legal-documents/no-hardcoded-values.test.ts.
25. The platform's privacy notice names {{publicListPeriod}} in §4 and in §7. It is filled with a period and its unit in the reader's language ('30 de zile', '30 days'), never a number written into the text. Merged with the deadlines, the list states and the list period, the notice carries no dotted blank (2026-09-25, DECISIONS.md §418). Verification: unit deadlines/duration-words.test.ts.
26. The platform's two group-run declarations name no identity document. The signing page therefore asks for none, and a signature under them stores none. The texts state the club's legitimate interest (art. 6(1)(f)), with art. 9(2)(f) for the health statement. They state the platform's seven days and the archive copy's three years. They list the rights of access, rectification, erasure, restriction and objection. A text that still names {{idDocument}} asks for it, and the club's copy masks it as before (2026-09-25, DECISIONS.md §418). Verification: unit legal-documents/group-run-declaration.test.ts, integration group-run-declarations/sign.test.ts.
27. A public registration, from the event's form or the family link, is refused while no terms version is in force, and refused without the terms tick. The tick is required, never folded, and names the version in force and the clauses accepted expressly (2026-09-25, `DECISIONS.md` §421).
28. The registration records the terms version in force and the moment of acceptance, and rewrites both when a cancelled registration is sent again. A staff entry records neither and is not refused, because the paper carries the terms (2026-09-25, `DECISIONS.md` §421).
29. A tick shown under a terms version that is no longer in force at submit is refused and names the tick. The form brings the box back unticked and says the terms changed. An empty or unreadable posted version counts as none and says nothing changed (2026-09-25, `DECISIONS.md` §421).
30. A terms version somebody accepted counts as relied on: its withdrawal and deletion are refused, as for a privacy notice with acknowledgements (2026-09-25, `DECISIONS.md` §421).
...
27. There are five legal document kinds: the privacy notice, the terms, and three declarations named "Declarație pe propria răspundere (concurs)" / "(alergare de grup, asfalt)" / "(alergare de grup, trail)" and their English twins. Each has a platform template in both languages whose only blanks are merge fields and the four club facts as <PLACEHOLDER>s. The sample seed wraps every kind in the not-approved banner everywhere but production, and production is refused (tests/unit/legal-documents/group-run-declaration.test.ts, tests/integration/legal/versions.test.ts).
28. A signature on a group-run declaration counts as reliance on its version, like an acceptance: the version cannot be edited or deleted under it (tests/integration/group-run-declarations/sign.test.ts).
...
36. Given a REGISTRATION_CONFIRMED message for an event, when it renders, then after the club's words and the QR/number card, and before the button and its links, each language half carries one facts block in that half's language. The rows are: Când (the date with its weekday, and the time or a race's two named times); Unde (the place's name in that language, its address, the map link); Program (the programme's timed rows); Traseu (surface, difficulty, distance, elevation and headlamp in the event page's own words and order, then the route link without a route description, the Strava event and the Facebook event); Cost (a paid event's amount and payment host, or a donation, its suggested amount and host); and Linkuri (#schedule with a programme, #rules with rules, #route only with a route description in that language, #links only when the page's own split leaves "Linkuri și fișiere" a link). A row the event has nothing for is not drawn: no Program row without timed rows, and no Cost row for a free or unstated cost. The list under the button does not repeat an anchor the block carries. While the place is to be announced, the Unde row is the page's sentence alone, and no name, address, map or programme place is in the message. The club's copy keeps the block without the QR, the code, a token or an attachment. The closed placeholder set is unchanged, and a text the club saved renders with the block and needs no edit (2026-09-25, `DECISIONS.md` §392).
37. Given an EVENT_REMINDER or a COMPLETE_DECLARATION (the participation confirmation, §104), when it renders, then it carries the same facts block, from the same function as the confirmation. On the reminder, the block replaces the programme sentence and the section links; the number, "what to bring", the code and the cancel line stay. Where the block is drawn, it replaces the one bold facts line of criterion 4; the number given by hand, the update notice, the organizer's message and "registration is open" keep that line. /admin/emails previews the block on all three messages, in both halves, from the sample event (2026-09-25, `DECISIONS.md` §392).
38. The reminder email of a night date carries one line with the sunset and to bring a light, in each half of a bilingual message, worded as a run on a group run. It reads the event's own end, so a daylight start whose duration carries it past dusk gets the line; no other message carries it (2026-09-25, `DECISIONS.md` §394).
39. Given an EVENT_REMINDER for an event starting within seven days, when it is rendered and Open-Meteo answers, then its facts block carries a «Vremea» / «Weather» row right under «Când»: the kind, temperature, chance of rain and wind, with «Prognoză: Open-Meteo» / «Forecast: Open-Meteo» as a word under them, each half of the bilingual message in its own language. Given a failed request, the reminder goes out without the row, and no other message type (the confirmation included) ever carries it (2026-09-25, `DECISIONS.md` §402).
40. Given an EVENT_REMINDER for an event starting within seven days, when it is rendered and Open-Meteo answers for the event's own place (the same rule and cached answer as the page), then its facts block carries a «Vremea» / «Weather» row right under «Când»: the kind, temperature, chance of rain and wind, with «Prognoză: Open-Meteo» / «Forecast: Open-Meteo» as a word under them, each half of the bilingual message in its own language, and never the page's details or hours. Given a failed request, the reminder goes out without the row, and no other message type (the confirmation included) ever carries it (2026-09-25, `DECISIONS.md` §416).
41. The address-confirmation email names the person whose registration it is, says where the data came from, states the link's lifetime from the club's "Termene" and says the registration lapses without it, in both languages (2026-09-25, `DECISIONS.md` §419).
42. The waiting-list offer names its deadline in the event's zone and a length that agrees with it. The length is the offer's own span from `offerCreatedAt` to `holdExpiresAt`: in hours from an hour up, and in minutes when the cap left less than an hour (a 20-minute offer says "20 de minute" / "20 minutes", never "o oră"). A resend after the deadline names the length only (2026-09-25, `DECISIONS.md` §419).
43. A declaration request or offer whose deadline is the event's start itself states the moment as «până la start, …» / "by the start, …" through `confirmationDueMoment` (2026-09-25, `DECISIONS.md` §419).
44. A minor's messages greet the parent or guardian and say whose registration they are about. The declaration request and the offer say who signs: both, each with their own identity document, when the declaration in force asks the minor too. The older DECLARATION_SIGNED message and every message with its own greeting (the link for another person included) keep their greeting (2026-09-25, `DECISIONS.md` §419).
45. The declaration request sent ahead of the participation window never says the race is free (2026-09-25, `DECISIONS.md` §419).
46. The update notice offers a way out through "Înscrierile mele", with no token (2026-09-25, `DECISIONS.md` §419).
47. The family link asks for the other person's agreement and points to the notice, and it does not say so at the address's limit (2026-09-25, `DECISIONS.md` §419).
48. The club's archive copies say how the identity document is masked and how long to keep the copy. Every period comes from the retention sweep's own constants, in duration words (2026-09-25, `DECISIONS.md` §419).
49. The group-run signer's PDF is masked like the club's copy and carries a "not you?" route to the contact page (2026-09-25, `DECISIONS.md` §419).
50. No club copy is queued for VERIFY_REGISTRATION_EMAIL or REGISTER_ANOTHER_PERSON. ORGANIZER_MESSAGE and EVENT_UPDATE_NOTICE get one club copy per send that says how many real participants received it and names nobody (2026-09-25, `DECISIONS.md` §419).
51. Every email the outbox sends carries a Reply-To of the addresses in force, comma-separated with the Gmail first. The From stays on the Mailgun domain whatever the setting.
...
15. A new event's cost defaults to FREE end to end: the create page preselects it, and a create posted with the cost box never opened saves costType FREE and reads it back — proven by a PGlite test through the create service, not only by source regexes and the e2e (2026-09-25, `DECISIONS.md` §398).
16. Given a message is sent, when the outbox records it, then the row carries the road that carried it (mailgun or gmail) and the recipients it reached; Mailgun's allowance, the "how many more registrations fit" forecast and "Trimite acum"'s stop count only Mailgun's rows, and the forecast names what Gmail carries instead (2026-09-26, `DECISIONS.md` §443).
17. Given 25 or more due rows whose road is Gmail and a newer due row whose road is Mailgun (a runner's address confirmation or a waiting-list offer), when the outbox next drains, then the Mailgun row is sent in that first drain: the worker claims Mailgun's rows apart from Gmail's, and Gmail's only as many as its pace lets one batch send (2026-09-26, `DECISIONS.md` §443).
...
4. Given an Administrator on /admin/emails, when they choose for each of the six groups (action links, confirmations, reminders, announcements, the club's mail, newsletter) Mailgun or the club's Gmail, Gmail's daily cap and pace, the choice at the cap and whether Mailgun's spent allowance overflows into Gmail, then the setting is saved with an audit row naming who changed it from what; any other role is refused on the server; a cap above 500 or a pace above 10 seconds is refused; the default sends the club's mail and the newsletter through Gmail and every participant group through Mailgun, defers at the cap, does not overflow, paces 6 seconds, and caps 200 recipients a day on production and 50 elsewhere (2026-09-26, `DECISIONS.md` §443).
5. Given a deployment without the contact form's Gmail account and app password, when any message is sent, then it takes Mailgun's road whatever the setting says, and every Mailgun figure counts it (2026-09-26, `DECISIONS.md` §443).
6. Given a message whose group goes through Gmail, when the environment captures it or the allowlist does not name its address or a copy, then it is captured or the copy dropped exactly as on Mailgun's road, it waits for nothing, and it is recorded as Gmail's with 0 recipients, counted against no cap (2026-09-26, `DECISIONS.md` §443).
7. Given Gmail has reached the club's cap in recipients (the address plus every copy) in the last rolling 24 hours, when a message of a Gmail group is next, then it is deferred to the moment the oldest send leaves the window when the club chose to wait, or sent through Mailgun at once when it chose so; never discarded; a message with more recipients than the whole cap goes through Mailgun (2026-09-26, `DECISIONS.md` §443).
8. Given two or more senders sharing one database — the drain after a response, the outbox job, "Trimite acum", in one instance or several — when each sends Gmail messages, then no two Gmail sends are closer than the club's pace: before every Gmail message the sender reads the rolling day's recipients and the last send from the database and holds its slot under a row lock, never with a transaction open during the SMTP call; a Gmail row's sent_at is the moment Gmail took it; a message whose wait exceeds the batch's 20 seconds of pacing is handed back without spending an attempt (2026-09-26, `DECISIONS.md` §443).
9. Given Gmail fails, when the failure happened before Gmail could have accepted the message (connection, authentication, DNS, TLS, envelope, message), then the message goes through Mailgun at once and Gmail is not asked again in that batch; when it may have been accepted (a timeout or a socket error), then the outbox retries it later rather than Mailgun sending a second copy; either way the failure's sanitized code and time are shown on /admin/emails and in /api/health's email block without changing the health status (2026-09-26, `DECISIONS.md` §443).
...
31. The signed declarations of a run are listed on its backoffice page for the Organizer and the Administrator only, by name and moment, never address or document. The fold is drawn only when the run offers the declaration or still keeps signatures. The PDF route answers 403 to any other role and 404 to a malformed id (2026-09-25, `DECISIONS.md` §393).
32. /admin/tasks's «Aplicația» / «The app» tab is reachable only by the roles the backoffice's own nav already offers it to (canManageRegistrations or canSeeDiagnostics), asserted on the server; a Voluntar gets a 404 on the route and the panel query alike, and a Tehnic reaches the tab from the backoffice's own nav, never only a typed address (2026-09-25, `DECISIONS.md` §397).
33. For a role without settings rights, a settings card that is a section of its own — the status, the cost, the links, the public participant list — is its heading and its closed line with nothing to open, and the sentence that the settings belong to an Organizer or an Administrator is said once; the server still refuses the change (2026-09-25, `DECISIONS.md` §406).
34. The registrations export (CSV and spreadsheet) ends with two columns, "Terms version" and "Terms accepted". They are blank, never 0, for a row that recorded no terms, and every earlier column keeps its position (§425).
35. Given a signed-in staff member, when they open `/admin/network` («Verificarea rețelei», linked from the guide and from the blocked-save warning), then the page tries from their browser a scripted save, a plain form post, a stored picture and the anti-robot check. Under each blocked row it names what IT should allow, with the host taken from the site's configuration and never a literal, and offers a copyable report holding only the results, the site's address and the browser's description. The page and its probe endpoint answer only signed-in staff, asserted on the server (2026-09-26, `DECISIONS.md` §436).
36. The club's checklist is written only by the Organizer, the Administrator and the Superadministrator, asserted in the service: the Redactor and Tehnic are refused FORBIDDEN and their attempt leaves no row and no audit entry. The Redactor and Tehnic can read the checklist but are offered no button to change it. The volunteer gets a 404 on `/admin/tasks`. «Club», «Anti-robot» and «Costuri» stay open to the Administrator and the Superadministrator only.
37. The registrations spreadsheet writes its timestamps (medically fit, submitted, confirmed, checked in) as the club's wall clock in Europe/Bucharest, in the 24-hour format dd.mm.yyyy hh:mm, never as the UTC clock.
38. Criterion 34: every backoffice job in `/admin/guide` is a title and numbered steps. Every «…» it quotes is a label the screen (or an email's button) shows in that language. The two languages are the same guide: same sections, roles, tasks, step counts, quotes and placeholders. The reader's own role's sections come first and open (2026-09-26, `DECISIONS.md` §441).
39. Criterion 35: the guide names roles, never a person. Its role table states each of the six roles' permissions as the server enforces them. A job that only a Superadministrator may do (every legal-text write, «Echipa») opens with «Rol necesar: Superadministrator.» / "Role needed: Superadministrator." (2026-09-26, `DECISIONS.md` §441).
40. Criterion 36: the guide's «Ce NU se poate face» lists the platform's hard limits: no edit of a verified email, no merge of registrations, no staff-signed declaration, no legal text in effect without an approved version, no registration past capacity. Its health step reads only the top-level `status` of /api/health (2026-09-26, `DECISIONS.md` §441).
...
13. After a message is sent, the contact page also says so in a toast ("Mesaj trimis clubului." / "Message sent to the club.") inside a polite status region, with a close button at least 44 px tall. The toast is the same whatever the message was classified as. A refresh of the page shows the banner and no toast.
14. Where the contact form is unavailable, the contact page shows every address in force under «Adresa de contact afișată» as its own mailto link, the club's Gmail first, joined by «sau» / "or".
15. Every email's Reply-To, the "or reply to this email" line, and the email editor's previews follow the address in force; with no setting saved, EMAIL_REPLY_TO as before.
...
9. The /devs status panel's «Starea în acest moment» (the «Sistem» tab of /admin/tasks) names the weather service, Open-Meteo, as public and keyless, and shows its last answer (when it was read and how many hours it holds) or, in amber, why it could not be read. Under the e2e stub it says the forecast is the fixed test hour. Verification: e2e `config-panels.spec.ts` renders the panel (§402).
10. The /devs status panel's «Starea în acest moment» (the «Sistem» tab of /admin/tasks) names the weather service, Open-Meteo, as public and keyless. It shows its last answer (when it was read, how many hours it holds, and how many places this instance read a forecast for in the last hour) or, in amber, why it could not be read. Under the e2e stub it says the forecast is the fixed test hour. Verification: e2e `config-panels.spec.ts` renders the panel (2026-09-25, `DECISIONS.md` §416).
...
12. Given an Administrator or a DEV-and-above role, when `/admin/tasks`'s «Aplicația» / «The app» tab is opened, then `docs/QUEUE.md` renders read-only with its own headings, in the page's own language, through the same renderer `/devs/docs` uses; a Tehnic reaches it from the backoffice's own tab bar as well as the address, and a role with no business on the page is refused. (2026-09-25, `DECISIONS.md` §397.)
13. The app tab on /admin/tasks: a Tehnic switching to it from the checkin nav lands on `/ro/admin/tasks`, asserted with `page.waitForURL` on the pathname followed by a direct `URL(page.url()).pathname` check, rather than a `$`-anchored regex that could match a transient URL mid-navigation on a shared machine (2026-09-25, `DECISIONS.md` §397).
14. Criterion 14 — the integration test covering a truncated WebP master keeps its cut length even by construction (`& ~1`), because an odd RIFF chunk size lacks the format's pad byte and makes libwebp refuse the header outright rather than exercising the full-decode failure path the test targets; this removes a length-parity dependency that could otherwise flip the test red on a different platform or a `sharp`/libwebp upgrade (2026-09-26, `DECISIONS.md` §430).
15. The starting «De făcut» list's 19 lines: three carry a due date — Amalia's Mailgun-plan-switch line (2026-11-01) and the two «după 10 octombrie» lines (2026-10-10) — everything else undated.
16. A tick, an untick or a move that changes nothing on the club's checklist shows the same info-level "nothing changed" notice the edit form already gives on an unchanged save, never a green success toast.
...
15. The /admin/tasks board carries a «Reînnoirea domeniului .com» row computed from DOMAIN_REGISTERED_ON plus DOMAIN_RENEWAL_YEARS (total years paid, default 1), counted on the club's calendar: done above 90 days before the expiry, open (amber) at 90 days or fewer, broken (red, never blocking) at 30 days or fewer and after the expiry date, and open with an explicit 'not configured' sentence while the registration date is unset; the row states the expiry date and the step to renew for several years and update DOMAIN_RENEWAL_YEARS (§435).
...
8. When `yarn check` runs, no Server Component passes a React element as a prop, other than `children`, to a client component. Client components here are anything from `@mui/*`, a `next` client component, next-intl's `Link`, or a local `"use client"` module. The one exception is a prop the installed library is shown, by its own source, to render as a child: `Alert`'s `action`. `yarn test:e2e:dev` runs on demand, and once a night on `qa` with no retry (`.github/workflows/e2e-dev-nightly.yml`), never on a pull request. It requests every backoffice route (as a Superadministrator) and every public route under `next dev`, twice each, and fails on any 5xx. Verification: unit `shared/server-element-props.test.ts`, `scripts/e2e-dev-nightly.test.ts`; e2e `dev-routes.spec.ts` with `E2E_DEV=1` (2026-09-24, `DECISIONS.md` §370; 2026-09-26, `DECISIONS.md` §426).
9. Given a migration numbered 0024 or later, when `yarn migrations:check` runs, then a `DROP INDEX` or `DROP CONSTRAINT` counts as a contract, unless the same file creates that name again or the constraint is a CHECK an earlier migration created. A file that drops one and adds anything is refused, and a file that drops one needs its `-- contract:` line. Verification: unit `scripts/migration-check.test.ts` (2026-09-26, `DECISIONS.md` §426).
10. Given `yarn ship`, when it waits for a pull request's checks, then it judges them only once none is pending and the same checks have been read twice in a row. It stops on any failed or cancelled check on the batch PR. On the release PR it stops on the same, except a Vercel deployment check, whose red it reports. Verification: unit `scripts/ship-checks.test.ts` (2026-09-26, `DECISIONS.md` §426).
11. Given `yarn docs:land`, when it reads a manifest item's results, then a blank fix report, a blank `decisionsTitle` or a blank CHANGELOG bullet stops it before any file is written. The bullet is the implementer's or the item's own `changelog`, never a fixer's. A fixer's "carried forward" or "no DECISIONS.md edit was made" text is not landed, and the dry run prints it. Verification: unit `scripts/land-entry.test.ts` (2026-09-26, `DECISIONS.md` §426).
...
7. 5 (amendment). The public cache's one-day ceiling is multiplied by the month's budget: twice at amber, four times at red. The level is read from the instance's memory without waiting on Neon. A write still expires what it changed at once (§333). Verification: unit `public-cache/cache.test.ts` (2026-09-26, `DECISIONS.md` §447).
8. Given a database that is away (Neon's quota refusal, a suspended or unreachable compute), when a public page is read, then it serves its last good copy with a bilingual notice. That covers the listing, an event page, the calendar, the standing pages, the gallery, the `.ics` feeds, the sitemap, the share and OG pictures, the header's menu, the deadlines and the contact switch. On Neon's quota refusal the copy may be up to a billing period old and the notice names the date the site is whole again; otherwise the limit is §281's twelve hours. A page never copied still shows the error page, never invented content. After the first failure a per-instance breaker stops asking the database for 15 s, doubling to 5 minutes. Verification: unit `resilience/last-good-resting.test.ts`, `resilience/breaker.test.ts`; integration `resilience/pages-resting-copy.test.ts` (2026-09-26, `DECISIONS.md` §447).
...
6. /admin/tasks shows the .com renewal row. It is green more than 90 days before the expiry computed from DOMAIN_REGISTERED_ON + DOMAIN_RENEWAL_YEARS, amber from 90 days, and red from 30 days with the chip «De reinnoit acum» / "Renew now" (never «Nu functioneaza» before the expiry). Unset dates show their own sentence.
7. /api/health reports degraded (503) from 30 days before the domain's expiry and after it. It stays unaffected while the dates are unset.
...
16. The emergency contact is erased seven days after the event, with the identity document and the health note, and counted on its own (2026-09-25, `DECISIONS.md` §421).
17. /api/health reports the domain's renewal as `domain: { status, expiresOn, daysLeft }` and answers degraded (HTTP 503) from 30 days before the expiry and after it; unset dates give `{ status: "unknown" }` and leave the health status unchanged (§435).
18. Given the month's budget read from Neon's API (never the database), when a job runs for real, then its quiet is planned under the longer of the Administrator's minimum interval and the governor's floor: 60 minutes at amber and 120 at red, riding §334's floor slots so the pings inside it open no connection. Once the quota reads spent, a ping answers 200 `reason: budget` without opening the pool. When the database is away (Neon's quota refusal, a refused or timed-out connection), a ping answers 200 `reason: database-away` and records the ping, and any other error still fails. The health checks widen their staleness by the governor's floor and by the interval the last run planned under. Verification: integration `jobs/job-budget.test.ts` (2026-09-26, `DECISIONS.md` §447).
