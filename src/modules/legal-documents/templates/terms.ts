...
export const termsEn: LegalDocumentBody = {
  sections: [
    ...
    },
    {
      ...
      paragraphs: [
        "Your name appears on an event's public participant list only if the club switches that list on for the event and you ticked that you want to when registering; the list shows the name from the form (or the display name, where allowed), the club and where the registration stands — {{participantListStates}} — nothing else; a cancelled or expired registration does not appear. We do not publish results with names yet; when we do, we will ask you separately. You withdraw your agreement to the list at any time from “My registrations”, or from the link in your confirmation email, without losing the registration.",
