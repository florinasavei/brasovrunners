...
import { type CurrentLegalDocument, findCurrentApprovedDocument } from "./repository";
...
/** One read per locale, straight from the database — what the sitemap lists. */
export async function legalDocumentsInForce<T extends Record<string, unknown>>(
  db: Database<T>,
  key: PublicLegalKey,
  now: Date,
): Promise<LegalInForce> {
 ...
 * (§281, the same keys the page body reads), so the head and the body agree during an
 * outage. `null` when the database is away and nothing is remembered — the metadata then
  ...
  try {
    ...
    for (const locale of routing.locales) {
      const read = await readWithLastGood(
        `legal:${key}:${locale}`,
        () => findCurrentApprovedDocument(getDb(), key, locale, now),
        now,
      );
