...
export function kindSummary(
  ...
  const links =
    readEventLinks(event?.links ?? null).length + (event?.stravaEventUrl ? 1 : 0) + (event?.facebookEventUrl ? 1 : 0);
  return join(words, [
    labels.type,
    labels.status,
    labels.surface,
...
/** Sub-card 1.3: `Strava · Facebook · 3 fișiere (GPX, Hartă, Rezultate)`, or `Niciun link`. */
