...
export default function CostFields({
   ...
   * show it (`DECISIONS.md` §390). `children`, not a named prop: a Server Component may hand a
  ...
  // form (`DECISIONS.md` §390): hidden, not removed, the same rule the two boxes above follow —
    ...
    <Box sx={{ display: shown ? "block" : "none" }}>
      <Stack direction={{ xs: "column", sm: "row" }} spacing={2}>
        ...
        {/* Hidden, not removed, while the club's discount strip is the one that matters
            ...
            `EXTERNAL` + `PAID` event now (`DECISIONS.md` §390 — the fee is paid at the
