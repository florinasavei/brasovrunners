 ...
 *
 * Every window this sweep enforces, in the order it runs them (§NNN):
 *
...
export const RETENTION = {
   ...
   * status (§NNN). The same ninety days as a sent message, because it is the same question.
   */
  orphanOutboxDays: 90,
  /**
   * An address never confirmed, a place never held: nothing to prove (§NNN).
   *
...
export type PruneCounts = {
  ...
  /** Messages about nobody, gone after `orphanOutboxDays` (§NNN). */
  orphanOutboxMessages: number;
  /** Registrations whose address lapsed unconfirmed, gone after `unconfirmedRegistrationDays` (§NNN). */
...
/** The counts, and the steps that threw (§NNN): an empty list is a sweep that did all of it. */
...
export async function pruneExpiredRows<T extends Record<string, unknown>>(
  ...
  // First (§NNN): seven days after the event, the identity document out of the declaration and
  // the health note out of the registration. The rows stay; the two fields go. The window whose
  ...
  /**
   * Thirty days after it stopped working, whichever way it stopped (§NNN): used, invalidated or
  ...
  // About nobody (§NNN): no registration and no participant will ever take these rows with it.
  ...
  /**
   * An address never confirmed, a place never held (§NNN). `status = EXPIRED` beside the reason,
...
/**
 * What `job_runs.last_error` says about a sweep that failed (§NNN): the step names and nothing
