...
export type RegistrationDetail = {
   ...
   * note itself never rides along with the page's main query (§NNN). What the withdrawal panel
   * reads to know whether there is anything to withdraw.
   */
  holdsHealthNote: boolean;
  /** The results consent (BR-REQ-072-01), no longer asked (§NNN) but withdrawable where given. */
  resultsNameConsent: boolean;
  /** For the link to everything held about this person (§NNN): the identity it is looked up by. */
...
/**
 * The four things somebody needs when a runner is on the ground (§NNN): how to reach them,
...
/**
 * One event's emergency sheet (§NNN): everyone confirmed — checked in or not, since a runner
...
/**
 * The extra columns the spreadsheet carries and the CSV does not (§NNN): sex, the age on race
