...
export async function requestMyRegistrationsLink<T extends Record<string, unknown>>(
  ...
  // The same hashed bucket as the other link request (§NNN): one mailbox, one allowance.
...
export type MyRegistration = {
  ...
  listed: boolean;
  /**
   * Whether there is a health note, or a Strava link or Instagram username, to withdraw (§NNN).
...
export async function listActiveRegistrationsForParticipant<T extends Record<string, unknown>>(
  ...
  const rows = await db
    .select({
      ...
      // Whether each is set, computed in SQL so the values never leave the database (§NNN).
