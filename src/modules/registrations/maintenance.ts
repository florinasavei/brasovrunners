...
export async function runRegistrationMaintenance<T extends Record<string, unknown>>(
   ...
   *
   * **It fails loudly (§NNN).** It used to be caught and forgotten, on the reasoning that a
  ...
  await finishJobRun(
    ...
    jobRunId,
    {
      ...
      errorCount,
      // The retention steps that failed, by name (§NNN) — the one error this run writes down,
