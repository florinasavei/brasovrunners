...
async function eraseRegistration<T extends Record<string, unknown>>(
  ...
  if (canTransition(current.status, "CANCELLED")) {
    const event = await eventForRegistration(db, current.eventId);
    // No "your registration is cancelled" to somebody who asked to be erased (§NNN).
  ...
  */
  ...
  await recordAuditEvent(db, {
    actorStaffUserId: actor.id,
    // Nobody, from the start (§NNN): the row records that a deletion happened and who
...
// --- The emergency details, and withdrawing consent (§NNN) --------------------------------------
...
/**
 * One event's emergency sheet (§NNN), under the same gate and with the same audit, once per
...
/**
 * Withdraw a participant's optional data on their behalf (§NNN; `AGENTS.md` §15.11) — the staff
