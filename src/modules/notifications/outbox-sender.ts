 ...
 * splits the rows between the two roads (§NNN), and the Reply-To both the sender and the renderer
 * set (§NNN, «Adresa de contact afișată») — the one place the three workers (the drain after a
 ...
 * before every Gmail message, so the pace and the cap hold across drains and instances (§NNN review).
 */
...
export async function createOutboxSender<T extends Record<string, unknown>>(
  ...
  // The Reply-To the club chose to show (§NNN), read once per batch; the renderer's "or reply" line follows it.
