...
export type TemplateContent = {
  ...
  footer?: string;
  /**
   * Who sends this and where the privacy notice is (§NNN) — the last line of every participant
...
export type TemplateData = {
  ...
  contactUrl?: string;
  /**
   * The privacy notice (§NNN), in the language of the half being built. Set by
  ...
  ro: {
    ...
    staffInvitation: {
      ...
      body: (d: TemplateData) => [
        ...
        // What the club keeps about its own team (§NNN), said to the person it is kept about.
    ...
    /** Who sends it, and the notice (§NNN); the address follows the sentence. */
...
export function buildTemplateContent(
  ...
  // The notice in this half's own language (§NNN): the Romanian half links /ro/…, the English /en/….
...
/**
 * The messages that are not to a participant about their own data (§NNN), and so carry no
...
/**
 * Who the controller is, as the privacy line names it (§NNN): the club's legal name from the
