 ...
 * shows (`readEmailVolumeToday`), counted the same way, from the same UTC boundaries.
 */
export async function readBulkLimit<T extends Record<string, unknown>>(db: Database<T>, now: Date): Promise<number | null> {
  const [due] = await db
  ...
  const [sent] = await db
    ...
    .from(emailOutbox)
    .where(and(gte(emailOutbox.sentAt, month), sql`${emailOutbox.sentAt} IS NOT NULL`));
