...
const PUBLIC_COLUMNS = {
  ...
  // The event's own reminder lead (§NNN), for the five steps' "we send a reminder … before".
...
export async function findEventNotificationDetails<T extends Record<string, unknown>>(
  ...
  const rows = await db
    .select({
      ...
      raceStartsAt: events.raceStartsAt,
      timezone: events.timezone,
      // The deadlines the words state (§NNN): this event's own reminder lead ("este peste 2
