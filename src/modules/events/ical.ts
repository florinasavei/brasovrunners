...
export type CalendarEvent = {
  ...
  costUrl?: string | null;
  /** Whether registration is with another organizer (§390) — a `PAID` cost then reads "la organizator". */
  registrationMode?: "NONE" | "INTERNAL" | "EXTERNAL" | null;
  /** The club's discount on an `EXTERNAL`-registration `PAID` event's own fee (§390), in the calendar's language. */
  ...
  */
  const costFacts = ((): string[] => {
    if (event.costType === "PAID" && event.registrationMode === "EXTERNAL") {
      // The place is entered and paid at the organizer's own form (§390), never the club's: its
      ...
      return event.discountNote ? [main, event.discountNote] : [main];
    }
