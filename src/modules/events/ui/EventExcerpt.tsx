...
export default function EventExcerpt({
  ...
  return (
    <Box sx={CARD_EXCERPT_SX} data-testid="card-excerpt">
      {before.length > 0 && <RichText body={docOf(before)} links={false} pictures="card" />}
      {words && (
        <Box sx={CARD_EXCERPT_WORDS_SX} data-testid="card-excerpt-words">
          <RichText body={docOf(words)} links={false} pictures="card" />
        </Box>
      )}
      {after.length > 0 && <RichText body={docOf(after)} links={false} pictures="card" />}
    </Box>
  );
}
