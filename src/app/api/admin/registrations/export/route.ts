...
/** The spreadsheet's own columns for one row (§NNN), blank when the row has none. */
function workbookExtras(details: WorkbookDetails | undefined): Pick<RegistrationSheetRow, "sex" | "ageOnRaceDay" | "nationality" | "city" | "tshirtSize"> {
  ...
  return {
    sex: details.sex,
    ageOnRaceDay: ageOn(details.birthDate, details.eventStartsAt),
...
export async function GET(request: Request): Promise<Response> {
  ...
  */
  ...
  /*
    The export is recorded (§NNN): who took a file of the club's participants, of which event, in
  ...
  */
  ...
  if (format === "xlsx") {
    ...
    // Sex, age, origin and t-shirt, for the sheet only (§NNN) — one query for the exported rows.
