...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  return {
    ...
    /*
      ...
      §NNN). The canonical carries no query — `?lista=`, `?interest=` and `?since=` are the same
      page. BR-REQ-040-01 criterion 5: each alternate points at *that locale's own slug*, looked
      up from the database — never this slug under another prefix, which is a 404 — and only a
      published locale appears (BR-REQ-040-02); `x-default` is the Romanian one.
    */
    alternates: pageAlternates(
      locale,
      slugRouteUrls(env.APP_BASE_URL, "/events/[slug]", await findPublishedTranslations(getDb(), event.id)),
    ),
    openGraph: {
      title: event.seoTitle ?? event.title,
      description: event.seoDescription ?? event.excerpt ?? undefined,
