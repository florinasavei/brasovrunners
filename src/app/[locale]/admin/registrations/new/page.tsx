...
export default async function NewRegistrationPage({ params, searchParams }: Props) {
  ...
  const now = new Date();
  // §NNN: whether the public lists show each runner's stage — the list tick's help says so then.
          ...
          */}
          ...
          <CheckboxField name="clubMemberDeclared">{rt("clubMemberDeclared", { club: CLUB_NAME })}</CheckboxField>
            ...
            <Box>
              ...
              {/* §NNN: said to the person on the phone too, once the notice in force describes it. */}
              {listStatesOn && (
