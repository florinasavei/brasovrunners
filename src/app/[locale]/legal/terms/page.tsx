...
import { deadlineMergeValues, publicListPeriodMergeValues } from "@/modules/legal-documents/domain/merge-fields";
    ...
    <Container id="main" component="main" maxWidth={PAGE_WIDTH} sx={{ py: { xs: DENSITY.pagePadY, sm: 3 } }}>
      ...
      {document ? (
        <>
          ...
          <LegalDocumentBody body={document.body} values={{ ...deadlineMergeValues(locale, await cachedDeadlines()), ...listStatesMergeValues(locale), ...publicListPeriodMergeValues(locale) }} emphasizeFilled={false} />
        </>
      ) : (
        <>
