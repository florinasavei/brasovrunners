...
test.describe.serial("BR-REQ-011-01 criterion 8 the route link", () => {
  test("is set in the editor and opens from the published event page", async ({ page }) => {
    ...
    // A route link the browser refuses, with its box shut again: pressing Salvează must open it
    // and put the cursor in the box, so a folded box is never a Save that silently does nothing
    // (§350).
