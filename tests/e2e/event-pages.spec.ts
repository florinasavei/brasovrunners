...
test.describe("BR-REQ-041-01 the event detail page on a phone", () => {
  ...
  test("draws the route as one row of pills, the cost as its own, «când» on one line", async ({ page }) => {
    ...
    // (`route-pills.ts` `srSuffix`, `DECISIONS.md` §394), so the regex anchors on the visible
    // word rather than comparing the element's whole text.
  ...
  test("does not push the header past a 320px viewport, and keeps it to one row", async ({ page }) => {
    ...
    */
    ...
    if (inHeader) {
      ...
      // the summary, "GDPR", the marks, a rule and the two flags (§378, §385, footer.spec.ts) — so the
      // language's centre is the bar's.
      const barBox = await page.locator("footer").boundingBox();
      expect(Math.abs(centre(languageBox) - centre(barBox))).toBeLessThan(4);
