 ...
 *
 * The seed publishes single-date events with no map link; a series card exists only when another
 * spec has made one (CI's seed has none), so the series and map checks run on whatever the listing
 * has, and the unit test carries the map link on both kinds of card.
 */
...
test.describe("BR-REQ-041-01 the listing's cards (§NNN)", () => {
  ...
  test("put a clock beside the time on every card's date line", async ({ page }) => {
    ...
    for (let i = 0; i < count; i += 1) {
      ...
      await expect(when.locator('svg[data-testid="CalendarMonthIcon"]')).toHaveCount(1);
