...
async function sign(page: Page, locale: "ro" | "en", name: string, email: string) {
  ...
  // The run's own minimum age (§NNN): the editor's default fourteen, stated in the text, and the
  // birth date asked for it — a native date box, typed as the value it posts.
  await expect(page.locator("#main")).toContainText(locale === "ro" ? "Declar că am cel puțin 14 ani." : "I declare that I am at least 14 years old.");
  await page.locator('input[name="birthDate"]').fill("1990-05-17");
