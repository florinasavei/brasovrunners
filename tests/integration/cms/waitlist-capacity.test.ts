 ...
 * service and the database alike; left alone by a caller that does not post it; refused on an
 * event that takes no registrations here, like the capacity; and carried by a repeat, a duplicate
 * and a series edit, like the capacity. Lowering it below the line removes nobody.
 */
...
describe("BR-REQ-035-01 saving the waiting list's length (§NNN)", () => {
  ...
  it("refuses a length on an event that takes no registrations here, as the capacity is refused", async () => {
    const event = await createDraft();
    const refusal = await refusalOf(
      save(event.id, event.version, { ...FIELDS(), registrationMode: "NONE", capacity: "", declarationDocumentId: "", waitlistCapacity: "5" }),
    );
    expect(refusal?.code).toBe("VALIDATION_ERROR");
    expect(refusal?.fields).toEqual(["registrationMode", "waitlistCapacity"]);
    // An empty box on such an event is fine: nothing to queue, nothing said.
    await save(event.id, event.version, { ...FIELDS(), registrationMode: "NONE", capacity: "", declarationDocumentId: "", waitlistCapacity: "" });
    expect((await reload(event.id)).waitlistCapacity).toBeNull();
  });
