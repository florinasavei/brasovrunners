...
describe("BR-REQ-050-01 the event's optional texts: both languages or neither (§352)", () => {
  it.each([
    ...
    // The route / training description in the "Traseul" card (§387).
...
describe("the club's discount on an EXTERNAL-registration PAID event's own fee, both languages or neither (§389)", () => {
  const EXTERNAL_PAID = {
