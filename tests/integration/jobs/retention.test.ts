...
describe("retention sweep", () => {
  ...
  it("deletes a token thirty days after it was used, and never one that is still live in somebody's inbox", async () => {
    await db.insert(emailActionTokens).values([
      {
        // Used thirty-one days ago, with an expiry still months ahead (§NNN): it can never be
  ...
  /**
   * §NNN — "an address never confirmed, a place never held: nothing to prove". A registration
  ...
  /**
   * §NNN — a message about nobody (no registration, no participant) has no row that will ever
  ...
  /**
   * §NNN — each step on its own, and the seven-day clearing first. A three-year step that throws
