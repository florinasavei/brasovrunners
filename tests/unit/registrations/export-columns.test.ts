...
/**
 * BR-REQ-031-05 criterion 4 — what leaves the application in a file (§322).
 *
...
describe("§322 the age column: whole years on the event's day", () => {
  const raceDay = new Date("2026-11-21T09:00:00.000Z");

  it("counts a birthday on the day itself, and not the day before", () => {
    expect(ageOnRaceDay("1990-11-21", raceDay, "Europe/Bucharest")).toBe(36);
    expect(ageOnRaceDay("1990-11-22", raceDay, "Europe/Bucharest")).toBe(35);
    expect(ageOnRaceDay("2008-11-21", raceDay, "Europe/Bucharest")).toBe(18);
  });

  it("is blank for no date, or one that cannot be read", () => {
    expect(ageOnRaceDay(null, raceDay, "Europe/Bucharest")).toBeNull();
    expect(ageOnRaceDay("not-a-date", raceDay, "Europe/Bucharest")).toBeNull();
  });
});
