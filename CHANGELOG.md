...
- Branding scaffold, all of it placeholder: `src/theme/brand.ts` is now the only file in `src/` that may name a colour, a font or a logo path, and the MUI theme is assembled from it. Two font *roles* rather than one family — display for headings, body for text — because a club typeface is usually good in a wordmark and tiring in a paragraph of event details at 320px; both resolve to Roboto until a real one arrives, so the swap changes headings without touching body text. A site header with the lockup, linking home, with a 44px tap target (BR-REQ-041-01 criterion 6). Placeholder mark and lockup under `public/brand/`, the mark duplicated as `src/app/icon.svg` for the browser tab, with a test that the two cannot drift apart — nothing imports a Next file-convention icon, so no compiler would notice.
- The palette pairs the pages actually render are asserted against WCAG AA (BR-REQ-070-02 criterion 4). That required a darker green for text and links: the primary green does not reach 4.5:1 on the page background, and a link is the smallest coloured text on the site. The full audit remains an e2e concern that is not built.
- The placeholder wordmark is one line, not two. Stacked inside a 32-unit box the comma below the Ș of BRAȘOV descends into the line beneath and reads as a stray accent on the next word — Romanian sits below the baseline as well as above it, and a lockup has to leave room for that.
...
- 102 new unit and integration tests, 217 in total. Nothing routes to any of the above yet: the registration form, the token pages and the job endpoint all wait on the domain and the club's approved texts.
