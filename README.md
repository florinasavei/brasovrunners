...
| [`scripts/ship-checks.mjs`](./scripts/ship-checks.mjs) | How `yarn ship` reads a pull request's checks: nothing is judged while one is pending, and the same checks with none pending must be read twice, 30 s apart, before a PR is called green or red (`docs/DISPATCHER.md`, `DECISIONS.md` §NNN) |
...
| [`scripts/land-entry.mjs`](./scripts/land-entry.mjs) | What `yarn docs:land` makes of one item's saved results — its DECISIONS title and body, its CHANGELOG bullet, its SPECS criteria — kept apart from `land-batch.mjs`, which writes files on import, so the rules are testable (`docs/DISPATCHER.md`, `DECISIONS.md` §NNN) |
...
| [`scripts/migration-check.mjs`](./scripts/migration-check.mjs) | `yarn migrations:check` — refuses a migration that both expands and contracts, and a contract migration without its `-- contract:` line; a leading `-- expand:` / `-- contract:` note overrides the classification; runs in `yarn check` (`AGENTS.md` §7.6, `DECISIONS.md` §NNN) |
| [`.github/workflows/e2e-dev-nightly.yml`](./.github/workflows/e2e-dev-nightly.yml) | Once a night on `qa`: the `next dev` walk of every backoffice and public route (`yarn test:e2e:dev`, §370) — minutes, so it is not on every pull request; a red run emails whoever last changed the schedule (`DECISIONS.md` §NNN) |
...
| [`vercel.json`](./vercel.json) | Vercel settings both projects read: feature branches create **no** deployment, so the Hobby plan's hundred a day is spent on `qa` and `main` only (`DECISIONS.md` §313) — both projects cancelled those builds anyway, but a cancelled build still counted |
...
| [`.nvmrc`](./.nvmrc) | Node version — the major, `22`; CI reads it via `node-version-file` (§125). `engines.node` states the supported major, `22.x` |
...
| Area | Choice | Notes |
| --- | --- | --- |
| Runtime | Node.js 22 | The major in `.nvmrc`, which CI reads; `engines.node` says the same, `22.x` (§125) |
