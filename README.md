...
| [`vercel.json`](./vercel.json) | Vercel settings both projects read: feature branches create **no** deployment, so the Hobby plan's hundred a day is spent on `qa` and `main` only (`DECISIONS.md` §NNN) — both projects cancelled those builds anyway, but a cancelled build still counted |
