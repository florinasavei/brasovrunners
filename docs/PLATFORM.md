...
| **Zoho Mail** | *planned* | Team mailboxes on `<domain>`, and a shared `contact@` — never application mail, which stays on a subdomain (`SETUP.md` §26, `DECISIONS.md` §56) | — | **Not created.** Free plan checked 2026-09-16: up to 5 users, 5 GB each, one domain, web access only |
