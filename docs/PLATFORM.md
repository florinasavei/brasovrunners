...
participanți" on `/admin/emails` adds one more per copied participant message — four per registration,
since the address-confirmation link is never copied — and `/admin/emails` and `/admin/tasks` count it. So the free plan supports roughly
